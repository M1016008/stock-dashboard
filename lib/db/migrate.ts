// lib/db/migrate.ts
// libsql ベースのスキーマ初期化。
// テーブルが無ければ CREATE する。drizzle-kit を別途実行しなくてもよい。
//
// schema.ts に新しいテーブルを追加したら、ここにも追記する。

import type { Client } from '@libsql/client'

const STATEMENTS = [
  `CREATE TABLE IF NOT EXISTS ohlcv (
    id INTEGER PRIMARY KEY AUTOINCREMENT,
    code TEXT NOT NULL,
    market TEXT NOT NULL,
    date TEXT NOT NULL,
    open REAL NOT NULL,
    high REAL NOT NULL,
    low REAL NOT NULL,
    close REAL NOT NULL,
    volume INTEGER NOT NULL,
    adjusted_close REAL,
    source TEXT NOT NULL,
    created_at TEXT NOT NULL
  )`,
  `CREATE TABLE IF NOT EXISTS hex_stages (
    id INTEGER PRIMARY KEY AUTOINCREMENT,
    date TEXT NOT NULL,
    code TEXT NOT NULL,
    name TEXT NOT NULL,
    sector_large TEXT,
    market_cap REAL,
    stage_a INTEGER NOT NULL,
    stage_b INTEGER,
    timeframe TEXT NOT NULL,
    close REAL,
    sma5 REAL,
    sma25 REAL,
    sma75 REAL,
    rsi14 REAL,
    macd REAL,
    created_at TEXT NOT NULL
  )`,
  `CREATE TABLE IF NOT EXISTS tv_indicators (
    id INTEGER PRIMARY KEY AUTOINCREMENT,
    code TEXT NOT NULL,
    date TEXT NOT NULL,
    timeframe TEXT NOT NULL,
    close REAL,
    sma5 REAL,
    sma25 REAL,
    sma75 REAL,
    sma200 REAL,
    ema5 REAL,
    ema25 REAL,
    rsi14 REAL,
    macd REAL,
    macd_signal REAL,
    macd_histogram REAL,
    bb_upper REAL,
    bb_middle REAL,
    bb_lower REAL,
    volume INTEGER,
    volume_ratio REAL,
    source TEXT NOT NULL,
    created_at TEXT NOT NULL
  )`,
  `CREATE TABLE IF NOT EXISTS tv_daily_snapshots (
    date TEXT NOT NULL,
    ticker TEXT NOT NULL,
    name TEXT NOT NULL,
    price REAL,
    currency TEXT,
    change_percent_1d REAL,
    volume_1d INTEGER,
    avg_volume_10d INTEGER,
    avg_volume_30d INTEGER,
    market_cap REAL,
    market_cap_currency TEXT,
    perf_pct_1w REAL,
    perf_pct_1m REAL,
    perf_pct_3m REAL,
    perf_pct_6m REAL,
    perf_pct_ytd REAL,
    sma_5d REAL,
    sma_25d REAL,
    sma_75d REAL,
    sma_150d REAL,
    sma_300d REAL,
    sma_5w REAL,
    sma_13w REAL,
    sma_25w REAL,
    sma_50w REAL,
    sma_100w REAL,
    sma_3m REAL,
    sma_5m REAL,
    sma_10m REAL,
    sma_20m REAL,
    sma_25m REAL,
    earnings_last_date TEXT,
    earnings_next_date TEXT,
    imported_at TEXT NOT NULL,
    source_file TEXT,
    PRIMARY KEY (date, ticker)
  )`,
  `CREATE INDEX IF NOT EXISTS idx_tv_snapshots_ticker ON tv_daily_snapshots(ticker)`,
  `CREATE INDEX IF NOT EXISTS idx_tv_snapshots_earnings_next ON tv_daily_snapshots(earnings_next_date)`,
  `CREATE TABLE IF NOT EXISTS csv_imports (
    id INTEGER PRIMARY KEY AUTOINCREMENT,
    date TEXT NOT NULL,
    file_name TEXT NOT NULL,
    row_count INTEGER NOT NULL,
    imported_at TEXT NOT NULL,
    detection_method TEXT
  )`,
  `CREATE TABLE IF NOT EXISTS sector_master (
    ticker TEXT PRIMARY KEY,
    name TEXT,
    sector_large TEXT,
    sector_small TEXT,
    updated_at TEXT NOT NULL,
    source_file TEXT
  )`,
  // ─── Phase 2: 銘柄ユニバース ───
  `CREATE TABLE IF NOT EXISTS ticker_universe (
    ticker TEXT PRIMARY KEY,
    name TEXT,
    active INTEGER NOT NULL DEFAULT 1,
    added_at INTEGER NOT NULL DEFAULT (unixepoch())
  )`,
  `CREATE TABLE IF NOT EXISTS historical_universe (
    ticker TEXT PRIMARY KEY,
    name TEXT,
    first_trade_date TEXT NOT NULL,
    last_trade_date TEXT NOT NULL,
    trading_days INTEGER NOT NULL DEFAULT 0,
    latest_ohlcv_date TEXT NOT NULL,
    is_latest_member INTEGER NOT NULL DEFAULT 0,
    status TEXT NOT NULL,
    missing_from_ticker_universe INTEGER NOT NULL DEFAULT 0,
    market_segment TEXT,
    sector17_code TEXT,
    sector17_name TEXT,
    sector33_code TEXT,
    sector33_name TEXT,
    margin_type TEXT,
    ohlcv_rows INTEGER NOT NULL DEFAULT 0,
    ml_physics_v2_rows INTEGER NOT NULL DEFAULT 0,
    has_ml_physics_v2 INTEGER NOT NULL DEFAULT 0,
    source TEXT NOT NULL DEFAULT 'ohlcv_daily',
    payload_json TEXT NOT NULL DEFAULT '{}',
    computed_at INTEGER NOT NULL DEFAULT (unixepoch())
  )`,
  `CREATE INDEX IF NOT EXISTS historical_universe_status_idx ON historical_universe(status, last_trade_date)`,
  `CREATE INDEX IF NOT EXISTS historical_universe_latest_idx ON historical_universe(is_latest_member, last_trade_date)`,
  `CREATE TABLE IF NOT EXISTS historical_universe_audits (
    id INTEGER PRIMARY KEY AUTOINCREMENT,
    run_started_at INTEGER NOT NULL,
    latest_ohlcv_date TEXT,
    ohlcv_ticker_count INTEGER NOT NULL DEFAULT 0,
    latest_member_count INTEGER NOT NULL DEFAULT 0,
    historical_only_count INTEGER NOT NULL DEFAULT 0,
    missing_ticker_universe_count INTEGER NOT NULL DEFAULT 0,
    ml_feature_complete_count INTEGER NOT NULL DEFAULT 0,
    ml_feature_incomplete_count INTEGER NOT NULL DEFAULT 0,
    ohlcv_row_count INTEGER NOT NULL DEFAULT 0,
    ml_feature_row_count INTEGER NOT NULL DEFAULT 0,
    status TEXT NOT NULL,
    payload_json TEXT NOT NULL,
    computed_at INTEGER NOT NULL DEFAULT (unixepoch())
  )`,
  // ─── Phase 2: 日足 OHLCV 履歴 ───
  `CREATE TABLE IF NOT EXISTS ohlcv_daily (
    ticker TEXT NOT NULL,
    date TEXT NOT NULL,
    open REAL NOT NULL,
    high REAL NOT NULL,
    low REAL NOT NULL,
    close REAL NOT NULL,
    volume INTEGER NOT NULL,
    PRIMARY KEY (ticker, date)
  )`,
  `CREATE INDEX IF NOT EXISTS ohlcv_date_idx ON ohlcv_daily(date)`,
  `CREATE INDEX IF NOT EXISTS ohlcv_date_ticker_idx ON ohlcv_daily(date, ticker)`,
  `CREATE TABLE IF NOT EXISTS manual_ohlcv_daily (
    market TEXT NOT NULL DEFAULT 'JP',
    ticker TEXT NOT NULL,
    date TEXT NOT NULL,
    open REAL NOT NULL,
    high REAL NOT NULL,
    low REAL NOT NULL,
    close REAL NOT NULL,
    volume INTEGER NOT NULL DEFAULT 0,
    source_name TEXT,
    source_note TEXT,
    imported_at INTEGER NOT NULL DEFAULT (unixepoch()),
    PRIMARY KEY (market, ticker, date)
  )`,
  `CREATE INDEX IF NOT EXISTS manual_ohlcv_market_ticker_date_idx ON manual_ohlcv_daily(market, ticker, date)`,
  `CREATE TABLE IF NOT EXISTS market_universe (
    market TEXT NOT NULL,
    ticker TEXT NOT NULL,
    name TEXT,
    active INTEGER NOT NULL DEFAULT 1,
    exchange TEXT,
    currency TEXT,
    asset_type TEXT,
    sector TEXT,
    industry TEXT,
    start_date TEXT,
    end_date TEXT,
    shares_outstanding INTEGER,
    source TEXT NOT NULL,
    source_payload_json TEXT NOT NULL DEFAULT '{}',
    updated_at INTEGER NOT NULL DEFAULT (unixepoch()),
    PRIMARY KEY (market, ticker)
  )`,
  `CREATE INDEX IF NOT EXISTS market_universe_market_active_idx ON market_universe(market, active, ticker)`,
  `CREATE INDEX IF NOT EXISTS market_universe_market_name_idx ON market_universe(market, name)`,
  `CREATE TABLE IF NOT EXISTS market_classifications (
    market TEXT NOT NULL,
    ticker TEXT NOT NULL,
    taxonomy TEXT NOT NULL,
    effective_from TEXT NOT NULL DEFAULT '0000-01-01',
    effective_to TEXT,
    sector_code TEXT,
    sector_name TEXT,
    industry_code TEXT,
    industry_name TEXT,
    source TEXT NOT NULL,
    confidence REAL NOT NULL DEFAULT 1,
    raw_json TEXT NOT NULL DEFAULT '{}',
    updated_at INTEGER NOT NULL DEFAULT (unixepoch()),
    PRIMARY KEY (market, ticker, taxonomy, effective_from)
  )`,
  `CREATE INDEX IF NOT EXISTS market_classifications_taxonomy_sector_idx ON market_classifications(market, taxonomy, sector_name, ticker)`,
  `CREATE INDEX IF NOT EXISTS market_classifications_taxonomy_industry_idx ON market_classifications(market, taxonomy, industry_name, ticker)`,
  `CREATE INDEX IF NOT EXISTS market_classifications_ticker_idx ON market_classifications(market, ticker)`,
  `CREATE TABLE IF NOT EXISTS market_ohlcv_daily (
    market TEXT NOT NULL,
    ticker TEXT NOT NULL,
    date TEXT NOT NULL,
    open REAL NOT NULL,
    high REAL NOT NULL,
    low REAL NOT NULL,
    close REAL NOT NULL,
    adj_open REAL,
    adj_high REAL,
    adj_low REAL,
    adj_close REAL,
    volume INTEGER NOT NULL,
    adj_volume INTEGER,
    div_cash REAL,
    split_factor REAL,
    source TEXT NOT NULL,
    imported_at INTEGER NOT NULL DEFAULT (unixepoch()),
    PRIMARY KEY (market, ticker, date)
  )`,
  `CREATE INDEX IF NOT EXISTS market_ohlcv_market_date_idx ON market_ohlcv_daily(market, date)`,
  `CREATE INDEX IF NOT EXISTS market_ohlcv_market_date_ticker_idx ON market_ohlcv_daily(market, date, ticker)`,
  `CREATE INDEX IF NOT EXISTS market_ohlcv_market_ticker_date_idx ON market_ohlcv_daily(market, ticker, date)`,
  `CREATE TABLE IF NOT EXISTS market_daily_snapshots (
    market TEXT NOT NULL,
    ticker TEXT NOT NULL,
    date TEXT NOT NULL,
    ma_5 REAL,
    ma_25 REAL,
    ma_75 REAL,
    ma_150 REAL,
    ma_300 REAL,
    weekly_ma_5 REAL,
    weekly_ma_13 REAL,
    weekly_ma_25 REAL,
    weekly_ma_50 REAL,
    weekly_ma_100 REAL,
    monthly_ma_3 REAL,
    monthly_ma_5 REAL,
    monthly_ma_10 REAL,
    monthly_ma_20 REAL,
    monthly_ma_25 REAL,
    daily_a_stage INTEGER,
    daily_b_stage INTEGER,
    weekly_a_stage INTEGER,
    weekly_b_stage INTEGER,
    monthly_a_stage INTEGER,
    monthly_b_stage INTEGER,
    prev_close REAL,
    close_5d REAL,
    close_20d REAL,
    close_60d REAL,
    close_120d REAL,
    avg_volume_20 REAL,
    ma_200 REAL,
    ma_200_prev REAL,
    ma_200_observations INTEGER,
    computed_at INTEGER NOT NULL DEFAULT (unixepoch()),
    PRIMARY KEY (market, ticker, date)
  )`,
  `CREATE INDEX IF NOT EXISTS market_snapshots_market_date_idx ON market_daily_snapshots(market, date)`,
  `CREATE INDEX IF NOT EXISTS market_snapshots_market_date_ticker_idx ON market_daily_snapshots(market, date, ticker)`,
  `CREATE INDEX IF NOT EXISTS market_snapshots_market_stage_idx ON market_daily_snapshots(
    market, daily_a_stage, daily_b_stage, weekly_a_stage, weekly_b_stage, monthly_a_stage, monthly_b_stage, date, ticker
  )`,
  `CREATE TABLE IF NOT EXISTS physical_momentum_metrics (
    market TEXT NOT NULL DEFAULT 'JP',
    symbol TEXT NOT NULL,
    date TEXT NOT NULL,
    velocity REAL,
    acceleration REAL,
    momentum REAL,
    force REAL,
    ma5_angle REAL,
    ma25_angle REAL,
    ma75_angle REAL,
    ma200_angle REAL,
    ma_angle_avg REAL,
    energy REAL,
    z_velocity REAL,
    z_acceleration REAL,
    z_momentum REAL,
    z_force REAL,
    z_ma_angle_avg REAL,
    z_energy REAL,
    physical_momentum_score REAL,
    physical_force_score REAL,
    physical_energy_score REAL,
    created_at INTEGER NOT NULL DEFAULT (unixepoch()),
    updated_at INTEGER NOT NULL DEFAULT (unixepoch()),
    PRIMARY KEY (market, symbol, date)
  )`,
  `CREATE INDEX IF NOT EXISTS physical_momentum_market_date_idx ON physical_momentum_metrics(market, date)`,
  `CREATE INDEX IF NOT EXISTS physical_momentum_market_score_idx ON physical_momentum_metrics(market, date, physical_momentum_score)`,
  `CREATE INDEX IF NOT EXISTS physical_momentum_symbol_date_idx ON physical_momentum_metrics(market, symbol, date)`,
  `CREATE TABLE IF NOT EXISTS trade_scenarios (
    id TEXT PRIMARY KEY,
    ticker TEXT NOT NULL,
    market TEXT NOT NULL DEFAULT 'JP',
    name TEXT,
    direction TEXT NOT NULL,
    status TEXT NOT NULL DEFAULT 'open',
    confidence TEXT NOT NULL DEFAULT 'medium',
    anchor_date TEXT NOT NULL,
    anchor_close REAL,
    horizon_days INTEGER NOT NULL,
    entry_plan_price REAL,
    target_price REAL,
    stop_loss_price REAL,
    thesis TEXT NOT NULL,
    invalidation TEXT,
    review_memo TEXT,
    selected_start_date TEXT,
    selected_end_date TEXT,
    source_range_label TEXT,
    context_json TEXT NOT NULL DEFAULT '{}',
    created_at TEXT NOT NULL,
    updated_at TEXT NOT NULL
  )`,
  `CREATE INDEX IF NOT EXISTS trade_scenarios_ticker_updated_idx ON trade_scenarios(market, ticker, updated_at)`,
  `CREATE INDEX IF NOT EXISTS trade_scenarios_status_idx ON trade_scenarios(status, updated_at)`,
  `CREATE INDEX IF NOT EXISTS trade_scenarios_anchor_idx ON trade_scenarios(market, ticker, anchor_date)`,
  `CREATE TABLE IF NOT EXISTS api_serving_cache (
    namespace TEXT NOT NULL,
    cache_key TEXT NOT NULL,
    payload_json TEXT NOT NULL,
    generated_at_ms INTEGER NOT NULL,
    expires_at INTEGER NOT NULL,
    updated_at INTEGER NOT NULL,
    PRIMARY KEY (namespace, cache_key)
  )`,
  `CREATE INDEX IF NOT EXISTS api_serving_cache_expires_idx
    ON api_serving_cache(namespace, expires_at)`,
  `CREATE TABLE IF NOT EXISTS market_data_runs (
    id INTEGER PRIMARY KEY AUTOINCREMENT,
    market TEXT NOT NULL,
    job_type TEXT NOT NULL,
    status TEXT NOT NULL,
    started_at INTEGER NOT NULL DEFAULT (unixepoch()),
    finished_at INTEGER,
    total_tickers INTEGER NOT NULL DEFAULT 0,
    succeeded INTEGER NOT NULL DEFAULT 0,
    failed INTEGER NOT NULL DEFAULT 0,
    rows_inserted INTEGER NOT NULL DEFAULT 0,
    error_summary TEXT,
    payload_json TEXT NOT NULL DEFAULT '{}'
  )`,
  `CREATE INDEX IF NOT EXISTS market_data_runs_latest_idx ON market_data_runs(market, job_type, started_at)`,
  `CREATE TABLE IF NOT EXISTS market_earnings_calendar (
    market TEXT NOT NULL,
    ticker TEXT NOT NULL,
    report_date TEXT NOT NULL,
    hour TEXT,
    time_bucket TEXT,
    fiscal_year INTEGER,
    fiscal_quarter INTEGER,
    eps_estimate REAL,
    eps_actual REAL,
    revenue_estimate REAL,
    revenue_actual REAL,
    source TEXT NOT NULL,
    source_url TEXT,
    raw_json TEXT NOT NULL DEFAULT '{}',
    imported_at INTEGER NOT NULL DEFAULT (unixepoch()),
    PRIMARY KEY (market, ticker, report_date)
  )`,
  `CREATE INDEX IF NOT EXISTS market_earnings_market_date_idx
    ON market_earnings_calendar(market, report_date, ticker)`,
  `CREATE INDEX IF NOT EXISTS market_earnings_market_ticker_date_idx
    ON market_earnings_calendar(market, ticker, report_date)`,
  `CREATE INDEX IF NOT EXISTS market_earnings_market_bucket_date_idx
    ON market_earnings_calendar(market, time_bucket, report_date)`,
  `CREATE TABLE IF NOT EXISTS sector_etf_holdings (
    id INTEGER PRIMARY KEY AUTOINCREMENT,
    etf_ticker TEXT NOT NULL,
    holding_ticker TEXT NOT NULL,
    holding_name TEXT NOT NULL,
    weight_pct REAL,
    shares REAL,
    market_value REAL,
    as_of_date TEXT,
    source TEXT NOT NULL,
    source_url TEXT,
    raw_json TEXT NOT NULL DEFAULT '{}',
    updated_at INTEGER NOT NULL DEFAULT (unixepoch())
  )`,
  `CREATE INDEX IF NOT EXISTS sector_etf_holdings_etf_weight_idx ON sector_etf_holdings(etf_ticker, weight_pct)`,
  `CREATE INDEX IF NOT EXISTS sector_etf_holdings_ticker_idx ON sector_etf_holdings(holding_ticker)`,
  `CREATE INDEX IF NOT EXISTS sector_etf_holdings_as_of_idx ON sector_etf_holdings(etf_ticker, as_of_date)`,
  `CREATE TABLE IF NOT EXISTS sector_etf_holding_runs (
    id INTEGER PRIMARY KEY AUTOINCREMENT,
    etf_ticker TEXT NOT NULL,
    source TEXT NOT NULL,
    status TEXT NOT NULL,
    started_at INTEGER NOT NULL DEFAULT (unixepoch()),
    finished_at INTEGER,
    holdings_count INTEGER NOT NULL DEFAULT 0,
    error_summary TEXT,
    source_url TEXT,
    payload_json TEXT NOT NULL DEFAULT '{}'
  )`,
  `CREATE INDEX IF NOT EXISTS sector_etf_holding_runs_latest_idx ON sector_etf_holding_runs(etf_ticker, started_at)`,
  `CREATE INDEX IF NOT EXISTS sector_etf_holding_runs_status_idx ON sector_etf_holding_runs(status, started_at)`,
  `CREATE TABLE IF NOT EXISTS jquants_daily_coverage (
    date TEXT PRIMARY KEY,
    expected_rows INTEGER NOT NULL,
    imported_at INTEGER NOT NULL DEFAULT (unixepoch())
  )`,
  `CREATE TABLE IF NOT EXISTS update_locks (
    job_type TEXT PRIMARY KEY,
    status TEXT NOT NULL,
    owner TEXT,
    started_at INTEGER NOT NULL DEFAULT (unixepoch()),
    heartbeat_at INTEGER NOT NULL DEFAULT (unixepoch()),
    lease_expires_at INTEGER NOT NULL
  )`,
  `CREATE TABLE IF NOT EXISTS jquants_sync_runs (
    id INTEGER PRIMARY KEY AUTOINCREMENT,
    target_date TEXT NOT NULL,
    api_type TEXT NOT NULL,
    expected_rows INTEGER NOT NULL DEFAULT 0,
    imported_rows INTEGER NOT NULL DEFAULT 0,
    missing_tickers TEXT,
    status TEXT NOT NULL,
    started_at INTEGER NOT NULL DEFAULT (unixepoch()),
    finished_at INTEGER,
    error_summary TEXT
  )`,
  `CREATE INDEX IF NOT EXISTS jquants_sync_target_idx ON jquants_sync_runs(target_date, api_type)`,
  `CREATE TABLE IF NOT EXISTS compute_state (
    job_type TEXT NOT NULL,
    ticker TEXT NOT NULL,
    last_processed_date TEXT,
    updated_at INTEGER NOT NULL DEFAULT (unixepoch()),
    PRIMARY KEY (job_type, ticker)
  )`,
  `CREATE INDEX IF NOT EXISTS compute_state_job_date_idx ON compute_state(job_type, last_processed_date)`,
  `CREATE TABLE IF NOT EXISTS dashboard_cache (
    date TEXT PRIMARY KEY,
    payload_json TEXT NOT NULL,
    computed_at INTEGER NOT NULL DEFAULT (unixepoch())
  )`,
  `CREATE TABLE IF NOT EXISTS dashboard_trade_signal_cache (
    cache_key TEXT PRIMARY KEY,
    payload_json TEXT NOT NULL,
    computed_at INTEGER NOT NULL DEFAULT (unixepoch())
  )`,
  `CREATE INDEX IF NOT EXISTS dashboard_trade_signal_cache_computed_idx ON dashboard_trade_signal_cache(computed_at DESC)`,
  `CREATE TABLE IF NOT EXISTS dashboard_earnings_alert_cache (
    cache_key TEXT PRIMARY KEY,
    payload_json TEXT NOT NULL,
    computed_at INTEGER NOT NULL DEFAULT (unixepoch())
  )`,
  `CREATE INDEX IF NOT EXISTS dashboard_earnings_alert_cache_computed_idx ON dashboard_earnings_alert_cache(computed_at DESC)`,
  `CREATE TABLE IF NOT EXISTS kabutan_material_news (
    article_id TEXT PRIMARY KEY,
    title TEXT NOT NULL,
    published_at TEXT NOT NULL,
    url TEXT NOT NULL,
    category TEXT NOT NULL DEFAULT '材料',
    snippet TEXT,
    related_tickers_json TEXT NOT NULL DEFAULT '[]',
    related_stocks_json TEXT NOT NULL DEFAULT '[]',
    source TEXT NOT NULL DEFAULT 'kabutan',
    fetched_at INTEGER NOT NULL DEFAULT (unixepoch()),
    parse_status TEXT NOT NULL DEFAULT 'ok',
    error_summary TEXT
  )`,
  `CREATE INDEX IF NOT EXISTS kabutan_material_news_published_idx ON kabutan_material_news(published_at DESC)`,
  `CREATE INDEX IF NOT EXISTS kabutan_material_news_status_idx ON kabutan_material_news(parse_status, fetched_at DESC)`,
  `CREATE TABLE IF NOT EXISTS kabutan_material_news_runs (
    id INTEGER PRIMARY KEY AUTOINCREMENT,
    status TEXT NOT NULL,
    started_at INTEGER NOT NULL DEFAULT (unixepoch()),
    finished_at INTEGER,
    fetched_count INTEGER NOT NULL DEFAULT 0,
    saved_count INTEGER NOT NULL DEFAULT 0,
    error_summary TEXT
  )`,
  `CREATE INDEX IF NOT EXISTS kabutan_material_news_runs_latest_idx ON kabutan_material_news_runs(started_at DESC)`,
  `CREATE TABLE IF NOT EXISTS kabutan_themes (
    theme_id TEXT PRIMARY KEY,
    name TEXT NOT NULL,
    rank INTEGER,
    ranking_period TEXT NOT NULL DEFAULT '3days_access',
    ranking_as_of TEXT,
    url TEXT NOT NULL,
    description TEXT,
    representative_stocks_json TEXT NOT NULL DEFAULT '[]',
    related_stocks_json TEXT NOT NULL DEFAULT '[]',
    stock_count INTEGER,
    source TEXT NOT NULL DEFAULT 'kabutan',
    fetched_at INTEGER NOT NULL DEFAULT (unixepoch()),
    parse_status TEXT NOT NULL DEFAULT 'ok',
    error_summary TEXT
  )`,
  `CREATE INDEX IF NOT EXISTS kabutan_themes_rank_idx ON kabutan_themes(rank, fetched_at DESC)`,
  `CREATE INDEX IF NOT EXISTS kabutan_themes_status_idx ON kabutan_themes(parse_status, fetched_at DESC)`,
  `CREATE TABLE IF NOT EXISTS kabutan_theme_runs (
    id INTEGER PRIMARY KEY AUTOINCREMENT,
    status TEXT NOT NULL,
    started_at INTEGER NOT NULL DEFAULT (unixepoch()),
    finished_at INTEGER,
    fetched_count INTEGER NOT NULL DEFAULT 0,
    saved_count INTEGER NOT NULL DEFAULT 0,
    error_summary TEXT
  )`,
  `CREATE INDEX IF NOT EXISTS kabutan_theme_runs_latest_idx ON kabutan_theme_runs(started_at DESC)`,
  `CREATE TABLE IF NOT EXISTS serving_daily_snapshot_dates (
    date TEXT PRIMARY KEY,
    tickers INTEGER NOT NULL,
    computed_at INTEGER NOT NULL DEFAULT (unixepoch())
  )`,
  // ─── Phase 2: 日次スナップショット (MA + ステージ) ───
  `CREATE TABLE IF NOT EXISTS daily_snapshots (
    ticker TEXT NOT NULL,
    date TEXT NOT NULL,
    ma_5 REAL,
    ma_25 REAL,
    ma_75 REAL,
    ma_150 REAL,
    ma_300 REAL,
    weekly_ma_5 REAL,
    weekly_ma_13 REAL,
    weekly_ma_25 REAL,
    weekly_ma_50 REAL,
    weekly_ma_100 REAL,
    monthly_ma_3 REAL,
    monthly_ma_5 REAL,
    monthly_ma_10 REAL,
    monthly_ma_20 REAL,
    monthly_ma_25 REAL,
    daily_a_stage INTEGER,
    daily_b_stage INTEGER,
    weekly_a_stage INTEGER,
    weekly_b_stage INTEGER,
    monthly_a_stage INTEGER,
    monthly_b_stage INTEGER,
    computed_at INTEGER NOT NULL DEFAULT (unixepoch()),
    PRIMARY KEY (ticker, date)
  )`,
  `CREATE INDEX IF NOT EXISTS snapshots_date_idx ON daily_snapshots(date)`,
  `CREATE INDEX IF NOT EXISTS snapshots_date_ticker_idx ON daily_snapshots(date, ticker)`,
  `CREATE INDEX IF NOT EXISTS snapshots_stage_pattern_idx ON daily_snapshots(
    daily_a_stage, daily_b_stage, weekly_a_stage, weekly_b_stage, monthly_a_stage, monthly_b_stage, date, ticker
  )`,
  `CREATE TABLE IF NOT EXISTS ma25m_monitor_daily (
    ticker TEXT NOT NULL,
    date TEXT NOT NULL,
    close REAL NOT NULL,
    ma25m REAL NOT NULL,
    distance_pct REAL NOT NULL,
    abs_distance_pct REAL NOT NULL,
    distance_1d_pct REAL,
    distance_3d_pct REAL,
    distance_5d_pct REAL,
    distance_10d_pct REAL,
    distance_20d_pct REAL,
    position_side TEXT NOT NULL,
    is_approaching INTEGER NOT NULL DEFAULT 0,
    approach_direction TEXT NOT NULL DEFAULT 'none',
    approach_speed_pct_per_day REAL NOT NULL DEFAULT 0,
    approach_consistency REAL NOT NULL DEFAULT 0,
    distance_shrink_5_pct REAL,
    distance_shrink_10_pct REAL,
    is_contact_default INTEGER NOT NULL DEFAULT 0,
    is_touch INTEGER NOT NULL DEFAULT 0,
    last_touch_date TEXT,
    touch_age_sessions INTEGER,
    cross_direction TEXT,
    last_cross_date TEXT,
    last_cross_direction TEXT,
    cross_age_sessions INTEGER,
    primary_status TEXT NOT NULL,
    closeness_score REAL NOT NULL,
    movement_score REAL NOT NULL,
    event_score REAL NOT NULL,
    approach_score REAL NOT NULL,
    is_rapid_approach INTEGER NOT NULL DEFAULT 0,
    computed_at INTEGER NOT NULL DEFAULT (unixepoch()),
    PRIMARY KEY (ticker, date)
  )`,
  `CREATE INDEX IF NOT EXISTS ma25m_monitor_date_score_idx ON ma25m_monitor_daily(date, approach_score)`,
  `CREATE INDEX IF NOT EXISTS ma25m_monitor_date_distance_idx ON ma25m_monitor_daily(date, abs_distance_pct)`,
  `CREATE INDEX IF NOT EXISTS ma25m_monitor_date_approach_idx ON ma25m_monitor_daily(date, is_approaching, approach_score)`,
  `CREATE INDEX IF NOT EXISTS ma25m_monitor_ticker_date_idx ON ma25m_monitor_daily(ticker, date)`,
  `CREATE TABLE IF NOT EXISTS ma25m_monitor_latest (
    ticker TEXT PRIMARY KEY,
    date TEXT NOT NULL,
    close REAL NOT NULL,
    ma25m REAL NOT NULL,
    distance_pct REAL NOT NULL,
    abs_distance_pct REAL NOT NULL,
    distance_1d_pct REAL,
    distance_3d_pct REAL,
    distance_5d_pct REAL,
    distance_10d_pct REAL,
    distance_20d_pct REAL,
    position_side TEXT NOT NULL,
    is_approaching INTEGER NOT NULL DEFAULT 0,
    approach_direction TEXT NOT NULL DEFAULT 'none',
    approach_speed_pct_per_day REAL NOT NULL DEFAULT 0,
    approach_consistency REAL NOT NULL DEFAULT 0,
    distance_shrink_5_pct REAL,
    distance_shrink_10_pct REAL,
    is_contact_default INTEGER NOT NULL DEFAULT 0,
    is_touch INTEGER NOT NULL DEFAULT 0,
    last_touch_date TEXT,
    touch_age_sessions INTEGER,
    cross_direction TEXT,
    last_cross_date TEXT,
    last_cross_direction TEXT,
    cross_age_sessions INTEGER,
    primary_status TEXT NOT NULL,
    closeness_score REAL NOT NULL,
    movement_score REAL NOT NULL,
    event_score REAL NOT NULL,
    approach_score REAL NOT NULL,
    is_rapid_approach INTEGER NOT NULL DEFAULT 0,
    computed_at INTEGER NOT NULL DEFAULT (unixepoch())
  )`,
  `CREATE INDEX IF NOT EXISTS ma25m_latest_date_score_idx ON ma25m_monitor_latest(date, approach_score)`,
  `CREATE INDEX IF NOT EXISTS ma25m_latest_date_distance_idx ON ma25m_monitor_latest(date, abs_distance_pct)`,
  `CREATE INDEX IF NOT EXISTS ma25m_latest_date_approach_idx ON ma25m_monitor_latest(date, is_approaching, approach_score)`,
  `CREATE TABLE IF NOT EXISTS monthly_ma_monitor_daily (
    ticker TEXT NOT NULL, period INTEGER NOT NULL, date TEXT NOT NULL,
    close REAL NOT NULL, ma_value REAL NOT NULL,
    distance_pct REAL NOT NULL, abs_distance_pct REAL NOT NULL,
    distance_1d_pct REAL, distance_3d_pct REAL, distance_5d_pct REAL,
    distance_10d_pct REAL, distance_20d_pct REAL,
    position_side TEXT NOT NULL,
    is_approaching INTEGER NOT NULL DEFAULT 0,
    approach_direction TEXT NOT NULL DEFAULT 'none',
    approach_speed_pct_per_day REAL NOT NULL DEFAULT 0,
    approach_consistency REAL NOT NULL DEFAULT 0,
    distance_shrink_5_pct REAL, distance_shrink_10_pct REAL,
    is_contact_default INTEGER NOT NULL DEFAULT 0,
    is_touch INTEGER NOT NULL DEFAULT 0,
    last_touch_date TEXT, touch_age_sessions INTEGER,
    cross_direction TEXT, last_cross_date TEXT,
    last_cross_direction TEXT, cross_age_sessions INTEGER,
    primary_status TEXT NOT NULL,
    closeness_score REAL NOT NULL, movement_score REAL NOT NULL,
    event_score REAL NOT NULL, approach_score REAL NOT NULL,
    is_rapid_approach INTEGER NOT NULL DEFAULT 0,
    computed_at INTEGER NOT NULL DEFAULT (unixepoch()),
    PRIMARY KEY (ticker, period, date)
  )`,
  `CREATE INDEX IF NOT EXISTS monthly_ma_monitor_date_period_score_idx ON monthly_ma_monitor_daily(date, period, approach_score)`,
  `CREATE INDEX IF NOT EXISTS monthly_ma_monitor_date_period_distance_idx ON monthly_ma_monitor_daily(date, period, abs_distance_pct)`,
  `CREATE INDEX IF NOT EXISTS monthly_ma_monitor_ticker_period_date_idx ON monthly_ma_monitor_daily(ticker, period, date)`,
  `CREATE TABLE IF NOT EXISTS monthly_ma_monitor_latest (
    ticker TEXT NOT NULL, period INTEGER NOT NULL, date TEXT NOT NULL,
    close REAL NOT NULL, ma_value REAL NOT NULL,
    distance_pct REAL NOT NULL, abs_distance_pct REAL NOT NULL,
    distance_1d_pct REAL, distance_3d_pct REAL, distance_5d_pct REAL,
    distance_10d_pct REAL, distance_20d_pct REAL,
    position_side TEXT NOT NULL,
    is_approaching INTEGER NOT NULL DEFAULT 0,
    approach_direction TEXT NOT NULL DEFAULT 'none',
    approach_speed_pct_per_day REAL NOT NULL DEFAULT 0,
    approach_consistency REAL NOT NULL DEFAULT 0,
    distance_shrink_5_pct REAL, distance_shrink_10_pct REAL,
    is_contact_default INTEGER NOT NULL DEFAULT 0,
    is_touch INTEGER NOT NULL DEFAULT 0,
    last_touch_date TEXT, touch_age_sessions INTEGER,
    cross_direction TEXT, last_cross_date TEXT,
    last_cross_direction TEXT, cross_age_sessions INTEGER,
    primary_status TEXT NOT NULL,
    closeness_score REAL NOT NULL, movement_score REAL NOT NULL,
    event_score REAL NOT NULL, approach_score REAL NOT NULL,
    is_rapid_approach INTEGER NOT NULL DEFAULT 0,
    computed_at INTEGER NOT NULL DEFAULT (unixepoch()),
    PRIMARY KEY (ticker, period)
  )`,
  `CREATE INDEX IF NOT EXISTS monthly_ma_latest_date_period_score_idx ON monthly_ma_monitor_latest(date, period, approach_score)`,
  `CREATE INDEX IF NOT EXISTS monthly_ma_latest_date_period_distance_idx ON monthly_ma_monitor_latest(date, period, abs_distance_pct)`,
  `CREATE TABLE IF NOT EXISTS monthly_ma_cluster_daily (
    ticker TEXT NOT NULL, cluster_key TEXT NOT NULL, date TEXT NOT NULL,
    periods_json TEXT NOT NULL, period_count INTEGER NOT NULL,
    cluster_type TEXT NOT NULL, is_strong INTEGER NOT NULL DEFAULT 0,
    band_low REAL NOT NULL, band_high REAL NOT NULL,
    band_average REAL NOT NULL, spread_pct REAL NOT NULL,
    close REAL NOT NULL, distance_pct REAL NOT NULL, abs_distance_pct REAL NOT NULL,
    approach_direction TEXT NOT NULL DEFAULT 'none',
    is_approaching INTEGER NOT NULL DEFAULT 0,
    approach_speed_pct_per_day REAL NOT NULL DEFAULT 0,
    is_touch INTEGER NOT NULL DEFAULT 0,
    last_touch_date TEXT, touch_age_sessions INTEGER,
    cross_direction TEXT, last_cross_date TEXT,
    last_cross_direction TEXT, cross_age_sessions INTEGER,
    primary_status TEXT NOT NULL, approach_score REAL NOT NULL,
    computed_at INTEGER NOT NULL DEFAULT (unixepoch()),
    PRIMARY KEY (ticker, cluster_key, date)
  )`,
  `CREATE INDEX IF NOT EXISTS monthly_ma_cluster_date_score_idx ON monthly_ma_cluster_daily(date, approach_score)`,
  `CREATE INDEX IF NOT EXISTS monthly_ma_cluster_ticker_date_idx ON monthly_ma_cluster_daily(ticker, date)`,
  `CREATE TABLE IF NOT EXISTS monthly_ma_cluster_latest (
    ticker TEXT NOT NULL, cluster_key TEXT NOT NULL, date TEXT NOT NULL,
    periods_json TEXT NOT NULL, period_count INTEGER NOT NULL,
    cluster_type TEXT NOT NULL, is_strong INTEGER NOT NULL DEFAULT 0,
    band_low REAL NOT NULL, band_high REAL NOT NULL,
    band_average REAL NOT NULL, spread_pct REAL NOT NULL,
    close REAL NOT NULL, distance_pct REAL NOT NULL, abs_distance_pct REAL NOT NULL,
    approach_direction TEXT NOT NULL DEFAULT 'none',
    is_approaching INTEGER NOT NULL DEFAULT 0,
    approach_speed_pct_per_day REAL NOT NULL DEFAULT 0,
    is_touch INTEGER NOT NULL DEFAULT 0,
    last_touch_date TEXT, touch_age_sessions INTEGER,
    cross_direction TEXT, last_cross_date TEXT,
    last_cross_direction TEXT, cross_age_sessions INTEGER,
    primary_status TEXT NOT NULL, approach_score REAL NOT NULL,
    is_cluster INTEGER NOT NULL DEFAULT 0,
    is_maximal INTEGER NOT NULL DEFAULT 0,
    computed_at INTEGER NOT NULL DEFAULT (unixepoch()),
    PRIMARY KEY (ticker, cluster_key)
  )`,
  `CREATE INDEX IF NOT EXISTS monthly_ma_cluster_latest_date_state_idx ON monthly_ma_cluster_latest(date, is_cluster, is_maximal, approach_score)`,
  // ─── Phase 2: バッチ実行履歴 ───
  `CREATE TABLE IF NOT EXISTS batch_runs (
    id INTEGER PRIMARY KEY AUTOINCREMENT,
    job_type TEXT NOT NULL,
    started_at INTEGER NOT NULL,
    finished_at INTEGER,
    status TEXT NOT NULL,
    total_tickers INTEGER,
    succeeded INTEGER DEFAULT 0,
    failed INTEGER DEFAULT 0,
    rows_inserted INTEGER DEFAULT 0,
    error_summary TEXT
  )`,
  // ─── Phase 3: 特徴量スナップショット (3 timescale × 30+ 次元) ───
  `CREATE TABLE IF NOT EXISTS feature_snapshots (
    ticker TEXT NOT NULL,
    date TEXT NOT NULL,
    timescale TEXT NOT NULL,
    bin_order_a_12 INTEGER, bin_order_a_13 INTEGER, bin_order_a_23 INTEGER,
    bin_order_b_12 INTEGER, bin_order_b_13 INTEGER, bin_order_b_23 INTEGER,
    rel_dist_a_12 REAL, rel_dist_a_13 REAL, rel_dist_a_23 REAL,
    rel_dist_b_12 REAL, rel_dist_b_13 REAL, rel_dist_b_23 REAL,
    divergence_a REAL, divergence_b REAL,
    divergence_a_delta REAL, divergence_b_delta REAL,
    fan_uniformity REAL, divergence_percentile REAL,
    stage_a_age INTEGER, stage_b_age INTEGER,
    slope_m1 REAL, accel_m1 REAL,
    slope_m2 REAL, accel_m2 REAL,
    slope_m3 REAL, accel_m3 REAL,
    slope_m4 REAL, accel_m4 REAL,
    slope_m5 REAL, accel_m5 REAL,
    angle_synchrony REAL,
    stage_a_oh TEXT, stage_b_oh TEXT,
    computed_at INTEGER NOT NULL DEFAULT (unixepoch()),
    PRIMARY KEY (ticker, date, timescale)
  )`,
  `CREATE INDEX IF NOT EXISTS feat_date_idx ON feature_snapshots(date)`,
  `CREATE INDEX IF NOT EXISTS feat_ts_idx ON feature_snapshots(timescale, date)`,
  // ─── Phase 3: フォワードリターン (4 horizon) ───
  `CREATE TABLE IF NOT EXISTS forward_returns (
    ticker TEXT NOT NULL,
    date TEXT NOT NULL,
    horizon_days INTEGER NOT NULL,
    return_pct REAL NOT NULL,
    return_category TEXT NOT NULL,
    end_date TEXT NOT NULL,
    PRIMARY KEY (ticker, date, horizon_days)
  )`,
  `CREATE INDEX IF NOT EXISTS fwd_date_idx ON forward_returns(date)`,
  `CREATE INDEX IF NOT EXISTS fwd_horizon_ticker_date_idx ON forward_returns(horizon_days, ticker, date)`,
  // ─── Phase 3: パターン統計 (6桁コード × horizon) ───
  `CREATE TABLE IF NOT EXISTS pattern_stats (
    pattern_code TEXT NOT NULL,
    horizon_days INTEGER NOT NULL,
    count INTEGER NOT NULL,
    p05 REAL, p25 REAL, p50 REAL, p75 REAL, p95 REAL,
    very_up_count INTEGER NOT NULL DEFAULT 0,
    up_count INTEGER NOT NULL DEFAULT 0,
    flat_count INTEGER NOT NULL DEFAULT 0,
    down_count INTEGER NOT NULL DEFAULT 0,
    very_down_count INTEGER NOT NULL DEFAULT 0,
    computed_at INTEGER NOT NULL DEFAULT (unixepoch()),
    PRIMARY KEY (pattern_code, horizon_days)
  )`,
  `CREATE INDEX IF NOT EXISTS pstat_code_idx ON pattern_stats(pattern_code)`,
  // ─── Phase 4: ステージ遷移カウント (6軸 × 6 from × 6 to = 216 行) ───
  `CREATE TABLE IF NOT EXISTS stage_transitions (
    axis TEXT NOT NULL,
    from_stage INTEGER NOT NULL,
    to_stage INTEGER NOT NULL,
    count INTEGER NOT NULL,
    computed_at INTEGER NOT NULL DEFAULT (unixepoch()),
    PRIMARY KEY (axis, from_stage, to_stage)
  )`,
  // ─── 業種・細分類の6ステージ構造集計 ───
  `CREATE TABLE IF NOT EXISTS sector_structure_daily (
    taxonomy TEXT NOT NULL,
    group_key TEXT NOT NULL,
    group_name TEXT NOT NULL,
    parent_group TEXT,
    date TEXT NOT NULL,
    n_stocks INTEGER NOT NULL,
    valid_stage_count INTEGER NOT NULL,
    strength_score REAL,
    transition_change_score REAL NOT NULL,
    momentum_5d REAL,
    momentum_10d REAL,
    momentum_20d REAL,
    propagation_direction TEXT NOT NULL DEFAULT 'neutral',
    propagation_phase INTEGER NOT NULL DEFAULT 0,
    propagation_label TEXT NOT NULL,
    improving_count INTEGER NOT NULL DEFAULT 0,
    deteriorating_count INTEGER NOT NULL DEFAULT 0,
    stable_count INTEGER NOT NULL DEFAULT 0,
    axis_json TEXT NOT NULL,
    composition_json TEXT NOT NULL,
    computed_at INTEGER NOT NULL DEFAULT (unixepoch()),
    PRIMARY KEY (taxonomy, group_key, date)
  )`,
  `CREATE INDEX IF NOT EXISTS sector_structure_taxonomy_date_idx
    ON sector_structure_daily(taxonomy, date)`,
  `CREATE INDEX IF NOT EXISTS sector_structure_date_momentum_idx
    ON sector_structure_daily(date, momentum_10d)`,
  // ─── Phase 4: 主要指数 OHLC (J-Quants /indices/bars/daily) ───
  `CREATE TABLE IF NOT EXISTS indices_daily (
    code TEXT NOT NULL,
    date TEXT NOT NULL,
    open REAL,
    high REAL,
    low REAL,
    close REAL,
    PRIMARY KEY (code, date)
  )`,
  `CREATE INDEX IF NOT EXISTS indices_date_idx ON indices_daily(date)`,
  // ─── Phase 4: 独自分類体系 (大分類 59 × 業種細分類 476) ───
  `CREATE TABLE IF NOT EXISTS stock_classification (
    ticker TEXT PRIMARY KEY,
    major_category TEXT NOT NULL,
    sub_industry TEXT NOT NULL,
    updated_at INTEGER NOT NULL DEFAULT (unixepoch())
  )`,
  `CREATE INDEX IF NOT EXISTS idx_classification_major ON stock_classification(major_category)`,
  `CREATE INDEX IF NOT EXISTS idx_classification_sub ON stock_classification(sub_industry)`,
  // ─── 会社四季報プロフィール ───
  `CREATE TABLE IF NOT EXISTS stock_shikiho_profiles (
    ticker TEXT PRIMARY KEY,
    forecast_per REAL,
    actual_pbr REAL,
    forecast_roe REAL,
    dividend_yield REAL,
    headline_1 TEXT,
    description_1 TEXT,
    headline_2 TEXT,
    description_2 TEXT,
    issue_label TEXT,
    release_date TEXT,
    company_feature TEXT,
    consolidated_business TEXT,
    updated_at INTEGER NOT NULL DEFAULT (unixepoch())
  )`,
  `CREATE INDEX IF NOT EXISTS idx_shikiho_profiles_issue
    ON stock_shikiho_profiles(issue_label)`,
  // ─── Phase 4: 信用残・空売り (J-Quants /markets/weekly_margin_interest) ───
  `CREATE TABLE IF NOT EXISTS weekly_margin_interest (
    ticker TEXT NOT NULL,
    date TEXT NOT NULL,
    long_margin REAL,
    short_margin REAL,
    long_change REAL,
    short_change REAL,
    imported_at INTEGER NOT NULL DEFAULT (unixepoch()),
    PRIMARY KEY (ticker, date)
  )`,
  `CREATE INDEX IF NOT EXISTS wmi_date_idx ON weekly_margin_interest(date)`,
  `CREATE INDEX IF NOT EXISTS wmi_ticker_date_idx ON weekly_margin_interest(ticker, date)`,
  `CREATE TABLE IF NOT EXISTS serving_margin_latest (
    ticker TEXT PRIMARY KEY,
    as_of_date TEXT NOT NULL,
    margin_type TEXT,
    long_margin REAL,
    short_margin REAL,
    long_change REAL,
    short_change REAL,
    credit_ratio REAL,
    short_ratio REAL,
    computed_at INTEGER NOT NULL DEFAULT (unixepoch())
  )`,
  `CREATE INDEX IF NOT EXISTS serving_margin_latest_date_idx ON serving_margin_latest(as_of_date)`,
  `CREATE INDEX IF NOT EXISTS serving_margin_latest_type_idx ON serving_margin_latest(margin_type, as_of_date)`,
  // ─── 企業概要: 財務サマリー / 日々公表信用残 ───
  `CREATE TABLE IF NOT EXISTS stock_financial_summaries (
    ticker TEXT NOT NULL,
    disclosure_no TEXT NOT NULL,
    disclosure_date TEXT NOT NULL,
    disclosure_time TEXT,
    document_type TEXT,
    period_type TEXT,
    period_start TEXT,
    period_end TEXT,
    fiscal_year_end TEXT,
    sales REAL,
    operating_profit REAL,
    ordinary_profit REAL,
    net_profit REAL,
    eps REAL,
    total_assets REAL,
    equity REAL,
    equity_ratio REAL,
    bps REAL,
    operating_cash_flow REAL,
    investing_cash_flow REAL,
    financing_cash_flow REAL,
    cash_equivalents REAL,
    annual_dividend REAL,
    payout_ratio REAL,
    forecast_sales REAL,
    forecast_operating_profit REAL,
    forecast_net_profit REAL,
    forecast_eps REAL,
    forecast_annual_dividend REAL,
    source TEXT NOT NULL DEFAULT 'jquants',
    imported_at INTEGER NOT NULL DEFAULT (unixepoch()),
    PRIMARY KEY (ticker, disclosure_no)
  )`,
  `CREATE INDEX IF NOT EXISTS stock_financial_date_idx
    ON stock_financial_summaries(ticker, disclosure_date DESC)`,
  `CREATE TABLE IF NOT EXISTS financial_disclosures (
    event_id TEXT PRIMARY KEY,
    ticker TEXT NOT NULL,
    published_at TEXT NOT NULL,
    source TEXT NOT NULL,
    disclosure_id TEXT NOT NULL,
    document_type TEXT,
    accounting_standard TEXT NOT NULL,
    correction_status TEXT NOT NULL,
    metadata_json TEXT NOT NULL DEFAULT '{}',
    imported_at INTEGER NOT NULL DEFAULT (unixepoch())
  )`,
  `CREATE INDEX IF NOT EXISTS financial_disclosures_ticker_published_idx
    ON financial_disclosures(ticker, published_at)`,
  `CREATE INDEX IF NOT EXISTS financial_disclosures_source_disclosure_idx
    ON financial_disclosures(source, disclosure_id)`,
  `CREATE TABLE IF NOT EXISTS normalized_financial_facts (
    fact_id TEXT PRIMARY KEY,
    event_id TEXT NOT NULL,
    ticker TEXT NOT NULL,
    published_at TEXT NOT NULL,
    period_start TEXT,
    period_end TEXT NOT NULL,
    fiscal_year_start TEXT,
    fiscal_year_end TEXT,
    target_fiscal_year INTEGER,
    period_kind TEXT NOT NULL,
    accumulation_kind TEXT NOT NULL,
    value_kind TEXT NOT NULL,
    metric TEXT NOT NULL,
    value REAL NOT NULL,
    unit TEXT NOT NULL,
    currency TEXT,
    consolidation_scope TEXT NOT NULL,
    accounting_standard TEXT NOT NULL,
    correction_status TEXT NOT NULL,
    source TEXT NOT NULL,
    source_field TEXT,
    disclosure_id TEXT NOT NULL,
    is_derived INTEGER NOT NULL DEFAULT 0,
    derivation_method TEXT,
    input_fact_ids_json TEXT NOT NULL DEFAULT '[]',
    definition_version TEXT,
    imported_at INTEGER NOT NULL DEFAULT (unixepoch())
  )`,
  `CREATE INDEX IF NOT EXISTS normalized_financial_facts_ticker_metric_period_idx
    ON normalized_financial_facts(ticker, metric, period_end, published_at)`,
  `CREATE INDEX IF NOT EXISTS normalized_financial_facts_event_idx
    ON normalized_financial_facts(event_id)`,
  `CREATE INDEX IF NOT EXISTS normalized_financial_facts_derivation_idx
    ON normalized_financial_facts(ticker, is_derived, accumulation_kind, period_end)`,
  `CREATE TABLE IF NOT EXISTS detailed_financial_facts (
    fact_id TEXT PRIMARY KEY,
    event_id TEXT NOT NULL,
    ticker TEXT NOT NULL,
    published_at TEXT NOT NULL,
    period_start TEXT,
    period_end TEXT NOT NULL,
    value_kind TEXT NOT NULL,
    metric TEXT NOT NULL,
    value REAL NOT NULL,
    unit TEXT NOT NULL,
    currency TEXT,
    consolidation_scope TEXT NOT NULL,
    accounting_standard TEXT NOT NULL,
    correction_status TEXT NOT NULL,
    source TEXT NOT NULL,
    source_concept TEXT NOT NULL,
    source_namespace TEXT,
    context_ref TEXT NOT NULL,
    dimensions_json TEXT NOT NULL DEFAULT '[]',
    disclosure_id TEXT NOT NULL,
    document_id TEXT,
    source_priority INTEGER NOT NULL,
    derivation_method TEXT,
    input_fact_ids_json TEXT NOT NULL DEFAULT '[]',
    definition_version TEXT,
    imported_at INTEGER NOT NULL DEFAULT (unixepoch())
  )`,
  `CREATE INDEX IF NOT EXISTS detailed_financial_facts_ticker_metric_period_idx
    ON detailed_financial_facts(ticker, metric, period_end, published_at)`,
  `CREATE INDEX IF NOT EXISTS detailed_financial_facts_disclosure_idx
    ON detailed_financial_facts(disclosure_id)`,
  `CREATE INDEX IF NOT EXISTS detailed_financial_facts_source_concept_idx
    ON detailed_financial_facts(source, source_concept)`,
  `CREATE TABLE IF NOT EXISTS financial_forecast_snapshots (
    snapshot_id TEXT PRIMARY KEY,
    event_id TEXT NOT NULL,
    ticker TEXT NOT NULL,
    published_at TEXT NOT NULL,
    target_fiscal_year INTEGER NOT NULL,
    target_period_start TEXT,
    target_period_end TEXT NOT NULL,
    forecast_scope TEXT NOT NULL,
    forecast_period TEXT NOT NULL,
    metric TEXT NOT NULL,
    value REAL NOT NULL,
    unit TEXT NOT NULL,
    currency TEXT,
    consolidation_scope TEXT NOT NULL,
    accounting_standard TEXT NOT NULL,
    source TEXT NOT NULL,
    source_field TEXT NOT NULL,
    disclosure_id TEXT NOT NULL,
    imported_at INTEGER NOT NULL DEFAULT (unixepoch())
  )`,
  `CREATE INDEX IF NOT EXISTS financial_forecast_snapshots_pit_idx
    ON financial_forecast_snapshots(ticker, metric, target_fiscal_year, published_at)`,
  `CREATE INDEX IF NOT EXISTS financial_forecast_snapshots_event_idx
    ON financial_forecast_snapshots(event_id)`,
  `CREATE TABLE IF NOT EXISTS calculated_financial_metrics (
    value_id TEXT PRIMARY KEY,
    ticker TEXT NOT NULL,
    as_of TEXT NOT NULL,
    period_start TEXT,
    period_end TEXT,
    metric TEXT NOT NULL,
    value REAL NOT NULL,
    unit TEXT NOT NULL,
    consolidation_scope TEXT NOT NULL,
    accounting_standard TEXT NOT NULL,
    definition_version TEXT NOT NULL,
    derivation_method TEXT NOT NULL,
    input_fact_ids_json TEXT NOT NULL,
    source TEXT NOT NULL DEFAULT 'calculated',
    computed_at INTEGER NOT NULL DEFAULT (unixepoch())
  )`,
  `CREATE INDEX IF NOT EXISTS calculated_financial_metrics_ticker_metric_asof_idx
    ON calculated_financial_metrics(ticker, metric, as_of)`,
  `CREATE TABLE IF NOT EXISTS valuation_metric_bases (
    basis_id TEXT PRIMARY KEY,
    ticker TEXT NOT NULL,
    effective_date TEXT NOT NULL,
    transforms_json TEXT NOT NULL,
    input_fact_ids_json TEXT NOT NULL,
    metric_definition_versions_json TEXT NOT NULL,
    forecast_snapshot_id TEXT,
    calculated_at INTEGER NOT NULL DEFAULT (unixepoch())
  )`,
  `CREATE INDEX IF NOT EXISTS valuation_metric_bases_ticker_date_idx
    ON valuation_metric_bases(ticker, effective_date)`,
  `CREATE TABLE IF NOT EXISTS valuation_daily_serving (
    ticker TEXT NOT NULL,
    valuation_date TEXT NOT NULL,
    price_date TEXT NOT NULL,
    price REAL NOT NULL,
    is_trading_day INTEGER NOT NULL DEFAULT 1,
    per REAL,
    forward_per REAL,
    pbr REAL,
    psr REAL,
    fcf_yield REAL,
    ev_ebitda REAL,
    net_debt REAL,
    dividend_yield REAL,
    roe REAL,
    revenue_growth REAL,
    enterprise_value REAL,
    ebitda REAL,
    peer_forward_per REAL,
    peer_pbr REAL,
    peer_fcf_yield REAL,
    peer_roe REAL,
    peer_revenue_growth REAL,
    peer_ev_ebitda REAL,
    basis_id TEXT NOT NULL,
    forecast_snapshot_id TEXT,
    calculated_at INTEGER NOT NULL DEFAULT (unixepoch()),
    PRIMARY KEY (ticker, valuation_date)
  )`,
  `CREATE INDEX IF NOT EXISTS valuation_daily_serving_date_ticker_idx
    ON valuation_daily_serving(valuation_date, ticker)`,
  `CREATE TABLE IF NOT EXISTS valuation_serving_checkpoints (
    ticker TEXT PRIMARY KEY,
    status TEXT NOT NULL,
    history_start_date TEXT,
    latest_valuation_date TEXT,
    latest_price_date TEXT,
    source_imported_at INTEGER NOT NULL DEFAULT 0,
    source_row_count INTEGER NOT NULL DEFAULT 0,
    definition_signature TEXT NOT NULL,
    persisted_rows INTEGER NOT NULL DEFAULT 0,
    error_message TEXT,
    calculated_at INTEGER NOT NULL DEFAULT (unixepoch()),
    updated_at INTEGER NOT NULL DEFAULT (unixepoch())
  )`,
  `CREATE INDEX IF NOT EXISTS valuation_serving_checkpoints_status_idx
    ON valuation_serving_checkpoints(status, ticker)`,
  `CREATE TABLE IF NOT EXISTS stock_screening_serving (
    ticker TEXT NOT NULL,
    as_of TEXT NOT NULL,
    snapshot_date TEXT NOT NULL,
    valuation_date TEXT,
    name TEXT NOT NULL,
    market_segment TEXT,
    sector17 TEXT,
    sector33 TEXT,
    major_category TEXT,
    sub_industry TEXT,
    price REAL,
    market_cap REAL,
    revenue REAL,
    revenue_growth REAL,
    eps_growth REAL,
    revenue_cagr_3y REAL,
    revenue_cagr_5y REAL,
    operating_margin REAL,
    roe REAL,
    roa REAL,
    equity_ratio REAL,
    standard_fcf REAL,
    fcf_yield REAL,
    net_debt REAL,
    roic REAL,
    per REAL,
    forward_per REAL,
    pbr REAL,
    psr REAL,
    ev_ebitda REAL,
    per_percentile_5y REAL,
    pbr_percentile_5y REAL,
    sector_valuation_percentile REAL,
    forecast_dividend_yield REAL,
    payout_ratio REAL,
    forecast_dps REAL,
    dps_yoy REAL,
    consecutive_increase_years INTEGER,
    consecutive_non_decrease_years INTEGER,
    dps_cagr_3y REAL,
    dps_cagr_5y REAL,
    forecast_revenue_growth REAL,
    forecast_operating_profit_growth REAL,
    forecast_eps_growth REAL,
    latest_forecast_revision_rate REAL,
    latest_forecast_revision_direction TEXT,
    has_current_forecast INTEGER,
    has_next_forecast INTEGER,
    daily_a_stage INTEGER,
    daily_b_stage INTEGER,
    weekly_a_stage INTEGER,
    weekly_b_stage INTEGER,
    monthly_a_stage INTEGER,
    monthly_b_stage INTEGER,
    stage_code TEXT,
    sector_structure_score REAL,
    sector_rank INTEGER,
    pms REAL,
    pfs REAL,
    ma_structure TEXT,
    short_term_check TEXT,
    short_term_score REAL,
    credit_ratio REAL,
    long_margin REAL,
    short_margin REAL,
    long_margin_change REAL,
    short_margin_change REAL,
    source_version TEXT NOT NULL,
    calculated_at INTEGER NOT NULL DEFAULT (unixepoch()),
    PRIMARY KEY (ticker, as_of)
  )`,
  `CREATE INDEX IF NOT EXISTS stock_screening_serving_asof_ticker_idx
    ON stock_screening_serving(as_of, ticker)`,
  `CREATE INDEX IF NOT EXISTS stock_screening_serving_growth_idx
    ON stock_screening_serving(as_of, revenue_growth, ticker)`,
  `CREATE INDEX IF NOT EXISTS stock_screening_serving_roe_idx
    ON stock_screening_serving(as_of, roe, ticker)`,
  `CREATE INDEX IF NOT EXISTS stock_screening_serving_forward_per_idx
    ON stock_screening_serving(as_of, forward_per, ticker)`,
  `CREATE INDEX IF NOT EXISTS stock_screening_serving_dividend_idx
    ON stock_screening_serving(as_of, forecast_dividend_yield, ticker)`,
  `CREATE INDEX IF NOT EXISTS stock_screening_serving_structure_idx
    ON stock_screening_serving(as_of, sector_structure_score, pms, ticker)`,
  `CREATE TABLE IF NOT EXISTS stock_screening_serving_dates (
    as_of TEXT PRIMARY KEY,
    snapshot_date TEXT NOT NULL,
    valuation_date TEXT,
    row_count INTEGER NOT NULL DEFAULT 0,
    source_version TEXT NOT NULL,
    status TEXT NOT NULL,
    error_message TEXT,
    calculated_at INTEGER NOT NULL DEFAULT (unixepoch())
  )`,
  `CREATE TABLE IF NOT EXISTS saved_screening_definitions (
    id TEXT PRIMARY KEY,
    name TEXT NOT NULL,
    definition_version INTEGER NOT NULL DEFAULT 1,
    state_json TEXT NOT NULL,
    conditions_json TEXT NOT NULL,
    condition_signature TEXT NOT NULL,
    active INTEGER NOT NULL DEFAULT 1,
    created_at INTEGER NOT NULL DEFAULT (unixepoch()),
    updated_at INTEGER NOT NULL DEFAULT (unixepoch())
  )`,
  `CREATE UNIQUE INDEX IF NOT EXISTS saved_screening_definitions_name_idx
    ON saved_screening_definitions(name)`,
  `CREATE INDEX IF NOT EXISTS saved_screening_definitions_active_idx
    ON saved_screening_definitions(active, updated_at)`,
  `CREATE TABLE IF NOT EXISTS saved_screening_evaluations (
    evaluation_id TEXT PRIMARY KEY,
    definition_id TEXT NOT NULL,
    definition_version INTEGER NOT NULL,
    evaluated_at INTEGER NOT NULL DEFAULT (unixepoch()),
    as_of TEXT NOT NULL,
    snapshot_date TEXT NOT NULL,
    previous_evaluation_id TEXT,
    previous_as_of TEXT,
    conditions_json TEXT NOT NULL,
    matched_count INTEGER NOT NULL DEFAULT 0,
    new_count INTEGER NOT NULL DEFAULT 0,
    stay_count INTEGER NOT NULL DEFAULT 0,
    out_count INTEGER NOT NULL DEFAULT 0,
    elapsed_ms INTEGER NOT NULL DEFAULT 0,
    status TEXT NOT NULL,
    error_message TEXT
  )`,
  `CREATE UNIQUE INDEX IF NOT EXISTS saved_screening_evaluations_definition_date_idx
    ON saved_screening_evaluations(definition_id, definition_version, as_of)`,
  `CREATE TABLE IF NOT EXISTS saved_screening_evaluation_members (
    evaluation_id TEXT NOT NULL,
    ticker TEXT NOT NULL,
    name TEXT NOT NULL,
    status TEXT NOT NULL,
    previous_values_json TEXT,
    current_values_json TEXT,
    changes_json TEXT,
    created_at INTEGER NOT NULL DEFAULT (unixepoch()),
    PRIMARY KEY (evaluation_id, ticker)
  )`,
  `CREATE INDEX IF NOT EXISTS saved_screening_evaluation_members_status_idx
    ON saved_screening_evaluation_members(evaluation_id, status, ticker)`,
  `CREATE TABLE IF NOT EXISTS trigger_definitions (
    id TEXT PRIMARY KEY,
    name TEXT NOT NULL,
    evaluation_config_json TEXT NOT NULL,
    view_config_json TEXT NOT NULL,
    evaluation_signature TEXT NOT NULL,
    evaluation_version INTEGER NOT NULL DEFAULT 1,
    engine_version INTEGER NOT NULL,
    score_version INTEGER NOT NULL,
    created_at INTEGER NOT NULL DEFAULT (unixepoch()),
    updated_at INTEGER NOT NULL DEFAULT (unixepoch()),
    archived_at INTEGER
  )`,
  `CREATE INDEX IF NOT EXISTS trigger_definitions_active_idx
    ON trigger_definitions(archived_at, updated_at DESC)`,
  `CREATE TABLE IF NOT EXISTS historical_trigger_scan_jobs (
    id TEXT PRIMARY KEY,
    status TEXT NOT NULL,
    request_json TEXT NOT NULL,
    request_signature TEXT NOT NULL,
    source_fingerprint TEXT NOT NULL,
    requested_start TEXT NOT NULL,
    requested_end TEXT NOT NULL,
    resolved_start TEXT,
    resolved_end TEXT,
    timeframe TEXT NOT NULL,
    created_at INTEGER NOT NULL DEFAULT (unixepoch()),
    started_at INTEGER,
    completed_at INTEGER,
    heartbeat_at INTEGER,
    total_trading_days INTEGER NOT NULL DEFAULT 0,
    processed_trading_days INTEGER NOT NULL DEFAULT 0,
    result_location TEXT,
    result_size_bytes INTEGER NOT NULL DEFAULT 0,
    duration_ms REAL NOT NULL DEFAULT 0,
    serialization_ms REAL NOT NULL DEFAULT 0,
    error_category TEXT,
    cancel_requested INTEGER NOT NULL DEFAULT 0,
    owner_token TEXT,
    attempt_count INTEGER NOT NULL DEFAULT 0,
    expires_at INTEGER NOT NULL
  )`,
  `CREATE INDEX IF NOT EXISTS historical_trigger_scan_jobs_status_created_idx
    ON historical_trigger_scan_jobs(status, created_at)`,
  `CREATE INDEX IF NOT EXISTS historical_trigger_scan_jobs_signature_idx
    ON historical_trigger_scan_jobs(request_signature, source_fingerprint, completed_at)`,
  `CREATE INDEX IF NOT EXISTS historical_trigger_scan_jobs_expires_idx
    ON historical_trigger_scan_jobs(expires_at)`,
  `CREATE TABLE IF NOT EXISTS trigger_outcome_analysis_jobs (
    id TEXT PRIMARY KEY,
    status TEXT NOT NULL,
    historical_scan_job_id TEXT NOT NULL,
    request_json TEXT NOT NULL,
    request_signature TEXT NOT NULL,
    source_fingerprint TEXT NOT NULL,
    analysis_cutoff_date TEXT,
    created_at INTEGER NOT NULL DEFAULT (unixepoch()),
    started_at INTEGER,
    completed_at INTEGER,
    heartbeat_at INTEGER,
    total_events INTEGER NOT NULL DEFAULT 0,
    processed_events INTEGER NOT NULL DEFAULT 0,
    total_tickers INTEGER NOT NULL DEFAULT 0,
    processed_tickers INTEGER NOT NULL DEFAULT 0,
    result_location TEXT,
    result_size_bytes INTEGER NOT NULL DEFAULT 0,
    duration_ms REAL NOT NULL DEFAULT 0,
    sql_ms REAL NOT NULL DEFAULT 0,
    calculation_ms REAL NOT NULL DEFAULT 0,
    aggregation_ms REAL NOT NULL DEFAULT 0,
    serialization_ms REAL NOT NULL DEFAULT 0,
    save_ms REAL NOT NULL DEFAULT 0,
    error_category TEXT,
    cancel_requested INTEGER NOT NULL DEFAULT 0,
    owner_token TEXT,
    attempt_count INTEGER NOT NULL DEFAULT 0,
    expires_at INTEGER NOT NULL
  )`,
  `CREATE INDEX IF NOT EXISTS trigger_outcome_analysis_jobs_status_created_idx
    ON trigger_outcome_analysis_jobs(status, created_at)`,
  `CREATE INDEX IF NOT EXISTS trigger_outcome_analysis_jobs_source_idx
    ON trigger_outcome_analysis_jobs(historical_scan_job_id, request_signature, source_fingerprint, completed_at)`,
  `CREATE INDEX IF NOT EXISTS trigger_outcome_analysis_jobs_expires_idx
    ON trigger_outcome_analysis_jobs(expires_at)`,
  `CREATE TABLE IF NOT EXISTS trigger_path_research_jobs (
    id TEXT PRIMARY KEY,
    status TEXT NOT NULL,
    outcome_job_id TEXT NOT NULL,
    historical_scan_job_id TEXT NOT NULL,
    source_fingerprint TEXT NOT NULL,
    analysis_cutoff_date TEXT NOT NULL,
    created_at INTEGER NOT NULL DEFAULT (unixepoch()),
    started_at INTEGER,
    completed_at INTEGER,
    heartbeat_at INTEGER,
    total_events INTEGER NOT NULL DEFAULT 0,
    processed_events INTEGER NOT NULL DEFAULT 0,
    total_tickers INTEGER NOT NULL DEFAULT 0,
    processed_tickers INTEGER NOT NULL DEFAULT 0,
    result_location TEXT,
    result_size_bytes INTEGER NOT NULL DEFAULT 0,
    duration_ms REAL NOT NULL DEFAULT 0,
    error_category TEXT,
    cancel_requested INTEGER NOT NULL DEFAULT 0,
    owner_token TEXT,
    attempt_count INTEGER NOT NULL DEFAULT 0,
    expires_at INTEGER NOT NULL
  )`,
  `CREATE INDEX IF NOT EXISTS trigger_path_research_jobs_status_created_idx
    ON trigger_path_research_jobs(status, created_at)`,
  `CREATE INDEX IF NOT EXISTS trigger_path_research_jobs_source_idx
    ON trigger_path_research_jobs(outcome_job_id, source_fingerprint, completed_at)`,
  `CREATE INDEX IF NOT EXISTS trigger_path_research_jobs_expires_idx
    ON trigger_path_research_jobs(expires_at)`,
  `CREATE TABLE IF NOT EXISTS trigger_ml_dataset_jobs (
    id TEXT PRIMARY KEY,
    status TEXT NOT NULL,
    path_research_job_id TEXT NOT NULL,
    outcome_job_id TEXT NOT NULL,
    historical_scan_job_id TEXT NOT NULL,
    source_fingerprint TEXT NOT NULL,
    split_policy_json TEXT NOT NULL,
    analysis_cutoff_date TEXT NOT NULL,
    created_at INTEGER NOT NULL DEFAULT (unixepoch()),
    started_at INTEGER,
    completed_at INTEGER,
    heartbeat_at INTEGER,
    total_events INTEGER NOT NULL DEFAULT 0,
    processed_events INTEGER NOT NULL DEFAULT 0,
    total_tickers INTEGER NOT NULL DEFAULT 0,
    processed_tickers INTEGER NOT NULL DEFAULT 0,
    result_location TEXT,
    result_size_bytes INTEGER NOT NULL DEFAULT 0,
    result_sha256 TEXT,
    duration_ms REAL NOT NULL DEFAULT 0,
    error_category TEXT,
    cancel_requested INTEGER NOT NULL DEFAULT 0,
    owner_token TEXT,
    attempt_count INTEGER NOT NULL DEFAULT 0
  )`,
  `CREATE INDEX IF NOT EXISTS trigger_ml_dataset_jobs_status_created_idx
    ON trigger_ml_dataset_jobs(status, created_at)`,
  `CREATE INDEX IF NOT EXISTS trigger_ml_dataset_jobs_source_idx
    ON trigger_ml_dataset_jobs(path_research_job_id, created_at)`,
  `CREATE TABLE IF NOT EXISTS trigger_evaluations (
    id TEXT PRIMARY KEY,
    definition_id TEXT NOT NULL,
    evaluation_version INTEGER NOT NULL,
    engine_version INTEGER NOT NULL,
    score_version INTEGER NOT NULL,
    run_signature TEXT NOT NULL,
    evaluation_config_signature TEXT NOT NULL,
    evaluation_config_snapshot_json TEXT NOT NULL,
    requested_as_of TEXT NOT NULL,
    resolved_as_of TEXT NOT NULL,
    started_at INTEGER NOT NULL DEFAULT (unixepoch()),
    completed_at INTEGER,
    status TEXT NOT NULL,
    pit_universe_count INTEGER NOT NULL DEFAULT 0,
    current_price_count INTEGER NOT NULL DEFAULT 0,
    stale_accepted_count INTEGER NOT NULL DEFAULT 0,
    trigger_evaluated_count INTEGER NOT NULL DEFAULT 0,
    trigger_matched_count INTEGER NOT NULL DEFAULT 0,
    final_matched_count INTEGER NOT NULL DEFAULT 0,
    duration_ms INTEGER NOT NULL DEFAULT 0,
    discovery_duration_ms INTEGER NOT NULL DEFAULT 0,
    snapshot_transform_ms INTEGER NOT NULL DEFAULT 0,
    persistence_duration_ms INTEGER NOT NULL DEFAULT 0,
    discovery_query_count INTEGER NOT NULL DEFAULT 0,
    discovery_db_query_ms REAL NOT NULL DEFAULT 0,
    attempt_count INTEGER NOT NULL DEFAULT 1,
    error_category TEXT,
    created_at INTEGER NOT NULL DEFAULT (unixepoch()),
    FOREIGN KEY (definition_id) REFERENCES trigger_definitions(id) ON DELETE RESTRICT
  )`,
  `CREATE UNIQUE INDEX IF NOT EXISTS trigger_evaluations_run_signature_idx
    ON trigger_evaluations(run_signature)`,
  `CREATE INDEX IF NOT EXISTS trigger_evaluations_definition_date_idx
    ON trigger_evaluations(definition_id, resolved_as_of DESC, created_at DESC)`,
  `CREATE TABLE IF NOT EXISTS trigger_evaluation_members (
    evaluation_id TEXT NOT NULL,
    ticker TEXT NOT NULL,
    company_name TEXT NOT NULL,
    market TEXT,
    trigger_status TEXT NOT NULL,
    price REAL NOT NULL,
    ma1_value REAL NOT NULL,
    ma2_value REAL NOT NULL,
    zone_upper REAL NOT NULL,
    zone_lower REAL NOT NULL,
    zone_distance_pct REAL NOT NULL,
    ma1_distance_pct REAL NOT NULL,
    ma2_distance_pct REAL NOT NULL,
    approach_velocity REAL NOT NULL,
    average_volume REAL,
    average_trading_value REAL,
    day_a_stage INTEGER,
    day_b_stage INTEGER,
    week_a_stage INTEGER,
    week_b_stage INTEGER,
    month_a_stage INTEGER,
    month_b_stage INTEGER,
    stage_available INTEGER NOT NULL,
    stage_complete INTEGER NOT NULL,
    trigger_score REAL NOT NULL,
    score_breakdown_json TEXT NOT NULL,
    price_date TEXT NOT NULL,
    ma_date TEXT NOT NULL,
    stage_date TEXT,
    price_freshness TEXT NOT NULL,
    price_staleness_sessions INTEGER NOT NULL,
    liquidity_lookback_sessions INTEGER NOT NULL,
    liquidity_observation_count INTEGER NOT NULL,
    liquidity_complete INTEGER NOT NULL,
    ma1_period INTEGER NOT NULL,
    ma2_period INTEGER NOT NULL,
    ma1_trend TEXT NOT NULL,
    ma2_trend TEXT NOT NULL,
    ma1_slope_pct REAL NOT NULL,
    ma2_slope_pct REAL NOT NULL,
    both_rising INTEGER NOT NULL,
    from_above INTEGER NOT NULL,
    approach_direction TEXT NOT NULL,
    ma_path TEXT NOT NULL,
    created_at INTEGER NOT NULL DEFAULT (unixepoch()),
    PRIMARY KEY (evaluation_id, ticker),
    FOREIGN KEY (evaluation_id) REFERENCES trigger_evaluations(id) ON DELETE CASCADE
  )`,
  `CREATE INDEX IF NOT EXISTS trigger_evaluation_members_status_idx
    ON trigger_evaluation_members(evaluation_id, trigger_status, ticker)`,
  `CREATE TABLE IF NOT EXISTS trigger_evaluation_observations (
    evaluation_id TEXT NOT NULL,
    ticker TEXT NOT NULL,
    disposition TEXT NOT NULL,
    exclusion_reason TEXT,
    trigger_status TEXT,
    price_position TEXT,
    both_rising INTEGER,
    from_above INTEGER,
    approach_direction TEXT,
    zone_upper REAL,
    zone_lower REAL,
    zone_distance_pct REAL,
    price REAL,
    trigger_score REAL,
    price_date TEXT,
    ma_date TEXT,
    stage_date TEXT,
    universe_filter_passed INTEGER NOT NULL,
    price_filter_passed INTEGER,
    liquidity_filter_passed INTEGER,
    stage_filter_passed INTEGER,
    data_available INTEGER NOT NULL,
    created_at INTEGER NOT NULL DEFAULT (unixepoch()),
    PRIMARY KEY (evaluation_id, ticker),
    FOREIGN KEY (evaluation_id) REFERENCES trigger_evaluations(id) ON DELETE CASCADE
  )`,
  `CREATE TABLE IF NOT EXISTS trigger_lifecycle_events (
    id TEXT PRIMARY KEY,
    definition_id TEXT NOT NULL,
    evaluation_version INTEGER NOT NULL,
    previous_evaluation_id TEXT,
    current_evaluation_id TEXT NOT NULL,
    ticker TEXT NOT NULL,
    event_type TEXT NOT NULL,
    previous_trigger_status TEXT,
    current_trigger_status TEXT,
    exit_reason TEXT,
    previous_score REAL,
    current_score REAL,
    previous_price REAL,
    current_price REAL,
    current_price_position TEXT,
    current_zone_upper REAL,
    current_zone_lower REAL,
    current_zone_distance_pct REAL,
    resolved_as_of TEXT NOT NULL,
    created_at INTEGER NOT NULL DEFAULT (unixepoch()),
    FOREIGN KEY (definition_id) REFERENCES trigger_definitions(id) ON DELETE RESTRICT,
    FOREIGN KEY (previous_evaluation_id) REFERENCES trigger_evaluations(id) ON DELETE RESTRICT,
    FOREIGN KEY (current_evaluation_id) REFERENCES trigger_evaluations(id) ON DELETE CASCADE
  )`,
  `CREATE UNIQUE INDEX IF NOT EXISTS trigger_lifecycle_events_identity_idx
    ON trigger_lifecycle_events(current_evaluation_id, ticker, event_type)`,
  `CREATE INDEX IF NOT EXISTS trigger_lifecycle_events_definition_date_idx
    ON trigger_lifecycle_events(definition_id, evaluation_version, resolved_as_of DESC)`,
  `CREATE TABLE IF NOT EXISTS trigger_evaluation_batches (
    id TEXT PRIMARY KEY,
    source TEXT NOT NULL,
    requested_as_of TEXT NOT NULL,
    resolved_as_of TEXT NOT NULL,
    started_at INTEGER NOT NULL DEFAULT (unixepoch()),
    completed_at INTEGER,
    status TEXT NOT NULL,
    owner_token TEXT,
    heartbeat_at INTEGER NOT NULL DEFAULT (unixepoch()),
    definition_count INTEGER NOT NULL DEFAULT 0,
    attempted_count INTEGER NOT NULL DEFAULT 0,
    completed_count INTEGER NOT NULL DEFAULT 0,
    reused_count INTEGER NOT NULL DEFAULT 0,
    failed_count INTEGER NOT NULL DEFAULT 0,
    skipped_count INTEGER NOT NULL DEFAULT 0,
    evaluation_count INTEGER NOT NULL DEFAULT 0,
    lifecycle_event_count INTEGER NOT NULL DEFAULT 0,
    duration_ms REAL NOT NULL DEFAULT 0,
    error_category TEXT,
    created_at INTEGER NOT NULL DEFAULT (unixepoch())
  )`,
  `CREATE UNIQUE INDEX IF NOT EXISTS trigger_evaluation_batches_source_date_idx
    ON trigger_evaluation_batches(source, resolved_as_of)`,
  `CREATE INDEX IF NOT EXISTS trigger_evaluation_batches_status_idx
    ON trigger_evaluation_batches(status, resolved_as_of DESC)`,
  `CREATE TABLE IF NOT EXISTS trigger_evaluation_batch_items (
    batch_id TEXT NOT NULL,
    definition_id TEXT NOT NULL,
    definition_name TEXT NOT NULL DEFAULT '',
    view_config_json TEXT NOT NULL DEFAULT '{"sort":null,"pageSize":50}',
    evaluation_version INTEGER NOT NULL,
    evaluation_config_signature TEXT NOT NULL,
    engine_version INTEGER NOT NULL,
    score_version INTEGER NOT NULL,
    evaluation_id TEXT,
    status TEXT NOT NULL,
    lifecycle_event_count INTEGER NOT NULL DEFAULT 0,
    duration_ms REAL NOT NULL DEFAULT 0,
    error_category TEXT,
    created_at INTEGER NOT NULL DEFAULT (unixepoch()),
    updated_at INTEGER NOT NULL DEFAULT (unixepoch()),
    PRIMARY KEY (batch_id, definition_id),
    FOREIGN KEY (batch_id) REFERENCES trigger_evaluation_batches(id) ON DELETE CASCADE,
    FOREIGN KEY (definition_id) REFERENCES trigger_definitions(id) ON DELETE RESTRICT
  )`,
  `CREATE INDEX IF NOT EXISTS trigger_evaluation_batch_items_status_idx
    ON trigger_evaluation_batch_items(batch_id, status, definition_id)`,
  `CREATE TABLE IF NOT EXISTS trigger_notification_settings (
    definition_id TEXT PRIMARY KEY,
    daily_digest_enabled INTEGER NOT NULL DEFAULT 0,
    lifecycle_alert_enabled INTEGER NOT NULL DEFAULT 0,
    max_candidates_in_digest INTEGER NOT NULL DEFAULT 10,
    include_new INTEGER NOT NULL DEFAULT 1,
    include_re_entry INTEGER NOT NULL DEFAULT 1,
    include_status_changed_to_near INTEGER NOT NULL DEFAULT 1,
    include_status_changed_to_in_zone INTEGER NOT NULL DEFAULT 1,
    include_rebounded INTEGER NOT NULL DEFAULT 1,
    include_broke_below_zone INTEGER NOT NULL DEFAULT 1,
    include_core_condition_exit INTEGER NOT NULL DEFAULT 0,
    include_stage_filter_exit INTEGER NOT NULL DEFAULT 0,
    include_universe_filter_exit INTEGER NOT NULL DEFAULT 0,
    include_data_unavailable INTEGER NOT NULL DEFAULT 0,
    include_other_exited INTEGER NOT NULL DEFAULT 0,
    created_at INTEGER NOT NULL DEFAULT (unixepoch()),
    updated_at INTEGER NOT NULL DEFAULT (unixepoch()),
    FOREIGN KEY (definition_id) REFERENCES trigger_definitions(id) ON DELETE CASCADE
  )`,
  `CREATE TABLE IF NOT EXISTS notification_delivery_profiles (
    id TEXT PRIMARY KEY,
    provider TEXT NOT NULL,
    recipient_email TEXT NOT NULL,
    enabled INTEGER NOT NULL DEFAULT 0,
    base_url TEXT NOT NULL,
    created_at INTEGER NOT NULL DEFAULT (unixepoch()),
    updated_at INTEGER NOT NULL DEFAULT (unixepoch())
  )`,
  `CREATE INDEX IF NOT EXISTS notification_delivery_profiles_provider_enabled_idx
    ON notification_delivery_profiles(provider, enabled, id)`,
  `CREATE TABLE IF NOT EXISTS notification_outbox (
    id TEXT PRIMARY KEY,
    batch_id TEXT NOT NULL,
    notification_type TEXT NOT NULL,
    status TEXT NOT NULL,
    notification_policy_version INTEGER NOT NULL,
    dedupe_key TEXT NOT NULL,
    subject TEXT NOT NULL,
    text_body TEXT NOT NULL,
    html_body TEXT NOT NULL,
    payload_json TEXT NOT NULL,
    created_at INTEGER NOT NULL DEFAULT (unixepoch()),
    ready_at INTEGER NOT NULL DEFAULT (unixepoch()),
    send_attempt_count INTEGER NOT NULL DEFAULT 0,
    sent_at INTEGER,
    last_error_category TEXT,
    delivery_profile_id TEXT,
    delivery_provider TEXT,
    provider_message_id TEXT,
    provider_thread_id TEXT,
    delivery_retryable INTEGER,
    delivery_started_at INTEGER,
    FOREIGN KEY (batch_id) REFERENCES trigger_evaluation_batches(id) ON DELETE RESTRICT
  )`,
  `CREATE UNIQUE INDEX IF NOT EXISTS notification_outbox_dedupe_idx
    ON notification_outbox(dedupe_key)`,
  `CREATE UNIQUE INDEX IF NOT EXISTS notification_outbox_batch_type_policy_idx
    ON notification_outbox(batch_id, notification_type, notification_policy_version)`,
  `CREATE INDEX IF NOT EXISTS notification_outbox_status_ready_idx
    ON notification_outbox(status, ready_at, created_at)`,
  `CREATE TABLE IF NOT EXISTS notification_delivery_attempts (
    id TEXT PRIMARY KEY,
    outbox_id TEXT NOT NULL,
    delivery_profile_id TEXT NOT NULL,
    attempt_number INTEGER NOT NULL,
    provider TEXT NOT NULL,
    started_at INTEGER NOT NULL DEFAULT (unixepoch()),
    completed_at INTEGER,
    status TEXT NOT NULL,
    error_category TEXT,
    retryable INTEGER,
    provider_message_id TEXT,
    provider_thread_id TEXT,
    duration_ms REAL NOT NULL DEFAULT 0,
    FOREIGN KEY (outbox_id) REFERENCES notification_outbox(id) ON DELETE RESTRICT,
    FOREIGN KEY (delivery_profile_id) REFERENCES notification_delivery_profiles(id) ON DELETE RESTRICT
  )`,
  `CREATE UNIQUE INDEX IF NOT EXISTS notification_delivery_attempts_outbox_attempt_idx
    ON notification_delivery_attempts(outbox_id, attempt_number)`,
  `CREATE INDEX IF NOT EXISTS notification_delivery_attempts_profile_started_idx
    ON notification_delivery_attempts(delivery_profile_id, started_at DESC)`,
  `CREATE TABLE IF NOT EXISTS trigger_notification_composition_runs (
    id TEXT PRIMARY KEY,
    batch_id TEXT NOT NULL,
    notification_policy_version INTEGER NOT NULL,
    started_at INTEGER NOT NULL DEFAULT (unixepoch()),
    completed_at INTEGER,
    status TEXT NOT NULL,
    created_count INTEGER NOT NULL DEFAULT 0,
    reused_count INTEGER NOT NULL DEFAULT 0,
    duration_ms REAL NOT NULL DEFAULT 0,
    error_category TEXT,
    FOREIGN KEY (batch_id) REFERENCES trigger_evaluation_batches(id) ON DELETE RESTRICT
  )`,
  `CREATE INDEX IF NOT EXISTS trigger_notification_composition_runs_batch_idx
    ON trigger_notification_composition_runs(batch_id, started_at DESC)`,
  `CREATE TABLE IF NOT EXISTS financial_foundation_backfill_checkpoints (
    checkpoint_type TEXT NOT NULL,
    checkpoint_key TEXT NOT NULL,
    status TEXT NOT NULL,
    source_rows INTEGER NOT NULL DEFAULT 0,
    persisted_rows INTEGER NOT NULL DEFAULT 0,
    details_json TEXT NOT NULL DEFAULT '{}',
    error_message TEXT,
    started_at INTEGER,
    finished_at INTEGER,
    updated_at INTEGER NOT NULL DEFAULT (unixepoch()),
    PRIMARY KEY (checkpoint_type, checkpoint_key)
  )`,
  `CREATE INDEX IF NOT EXISTS financial_foundation_backfill_status_idx
    ON financial_foundation_backfill_checkpoints(checkpoint_type, status, checkpoint_key)`,
  `CREATE TABLE IF NOT EXISTS daily_margin_alerts (
    ticker TEXT NOT NULL,
    publication_date TEXT NOT NULL,
    application_date TEXT NOT NULL,
    publication_reason_json TEXT,
    short_outstanding REAL,
    short_change REAL,
    short_ratio REAL,
    long_outstanding REAL,
    long_change REAL,
    long_ratio REAL,
    short_long_ratio REAL,
    short_negotiable REAL,
    short_standardized REAL,
    long_negotiable REAL,
    long_standardized REAL,
    regulation_class TEXT,
    source TEXT NOT NULL DEFAULT 'jquants',
    imported_at INTEGER NOT NULL DEFAULT (unixepoch()),
    PRIMARY KEY (ticker, publication_date, application_date)
  )`,
  `CREATE INDEX IF NOT EXISTS daily_margin_alert_ticker_date_idx
    ON daily_margin_alerts(ticker, application_date DESC, publication_date DESC)`,
  `CREATE INDEX IF NOT EXISTS daily_margin_alert_date_idx
    ON daily_margin_alerts(application_date DESC)`,
  // ─── 企業概要: EDINET 有価証券報告書 / 大量保有報告書 ───
  `CREATE TABLE IF NOT EXISTS edinet_documents (
    document_id TEXT PRIMARY KEY,
    ticker TEXT,
    document_type TEXT NOT NULL,
    submitted_at TEXT,
    period_end TEXT,
    filer_name TEXT,
    status TEXT NOT NULL DEFAULT 'pending',
    error_message TEXT,
    processed_at INTEGER,
    imported_at INTEGER NOT NULL DEFAULT (unixepoch())
  )`,
  `CREATE INDEX IF NOT EXISTS edinet_documents_ticker_type_idx
    ON edinet_documents(ticker, document_type, submitted_at DESC)`,
  `CREATE TABLE IF NOT EXISTS edinet_company_snapshots (
    document_id TEXT PRIMARY KEY,
    ticker TEXT NOT NULL,
    edinet_code TEXT,
    document_type TEXT NOT NULL,
    filer_name TEXT,
    published_at TEXT NOT NULL,
    period_start TEXT,
    period_end TEXT,
    fiscal_year INTEGER,
    accounting_standard TEXT NOT NULL DEFAULT 'UNKNOWN',
    correction_status TEXT NOT NULL DEFAULT 'original',
    consolidation_scope TEXT NOT NULL DEFAULT 'mixed',
    company_overview_json TEXT NOT NULL DEFAULT '{}',
    employee_information_json TEXT NOT NULL DEFAULT '{}',
    officer_information_json TEXT NOT NULL DEFAULT '{}',
    segment_information_json TEXT NOT NULL DEFAULT '{}',
    has_company_overview INTEGER NOT NULL DEFAULT 0,
    employee_snapshot_count INTEGER NOT NULL DEFAULT 0,
    officer_count INTEGER NOT NULL DEFAULT 0,
    segment_count INTEGER NOT NULL DEFAULT 0,
    geographic_area_count INTEGER NOT NULL DEFAULT 0,
    major_customer_count INTEGER NOT NULL DEFAULT 0,
    major_shareholder_count INTEGER NOT NULL DEFAULT 0,
    policy_holding_count INTEGER NOT NULL DEFAULT 0,
    xbrl_fact_count INTEGER NOT NULL DEFAULT 0,
    parser_version TEXT NOT NULL,
    source TEXT NOT NULL DEFAULT 'edinet',
    imported_at INTEGER NOT NULL DEFAULT (unixepoch())
  )`,
  `CREATE INDEX IF NOT EXISTS edinet_company_snapshots_ticker_pit_idx
    ON edinet_company_snapshots(ticker, published_at DESC, period_end DESC)`,
  `CREATE TABLE IF NOT EXISTS major_shareholders (
    ticker TEXT NOT NULL,
    document_id TEXT NOT NULL,
    rank INTEGER NOT NULL,
    fiscal_year_end TEXT,
    holder_name TEXT NOT NULL,
    shares REAL,
    holding_ratio REAL,
    submitted_at TEXT,
    source TEXT NOT NULL DEFAULT 'edinet',
    imported_at INTEGER NOT NULL DEFAULT (unixepoch()),
    PRIMARY KEY (ticker, document_id, rank)
  )`,
  `CREATE INDEX IF NOT EXISTS major_shareholders_ticker_date_idx
    ON major_shareholders(ticker, fiscal_year_end DESC, rank)`,
  `CREATE TABLE IF NOT EXISTS policy_holdings (
    ticker TEXT NOT NULL,
    document_id TEXT NOT NULL,
    rank INTEGER NOT NULL,
    fiscal_year_end TEXT,
    issuer_name TEXT NOT NULL,
    shares REAL,
    book_value REAL,
    purpose TEXT,
    quantitative_effect TEXT,
    holding_type TEXT,
    submitted_at TEXT,
    source TEXT NOT NULL DEFAULT 'edinet',
    imported_at INTEGER NOT NULL DEFAULT (unixepoch()),
    PRIMARY KEY (ticker, document_id, rank)
  )`,
  `CREATE INDEX IF NOT EXISTS policy_holdings_ticker_date_idx
    ON policy_holdings(ticker, fiscal_year_end DESC, book_value DESC)`,
  `CREATE TABLE IF NOT EXISTS large_holding_reports (
    ticker TEXT NOT NULL,
    document_id TEXT NOT NULL,
    submitted_at TEXT,
    report_date TEXT,
    holder_name TEXT,
    shares REAL,
    holding_ratio REAL,
    previous_holding_ratio REAL,
    purpose TEXT,
    report_kind TEXT,
    source TEXT NOT NULL DEFAULT 'edinet',
    imported_at INTEGER NOT NULL DEFAULT (unixepoch()),
    PRIMARY KEY (ticker, document_id)
  )`,
  `CREATE INDEX IF NOT EXISTS large_holding_reports_ticker_date_idx
    ON large_holding_reports(ticker, submitted_at DESC)`,
  `CREATE TABLE IF NOT EXISTS large_holder_filings (
    document_id TEXT PRIMARY KEY,
    filing_type TEXT NOT NULL,
    submitted_at TEXT NOT NULL,
    obligation_date TEXT,
    reference_date TEXT,
    parent_document_id TEXT,
    corrected_document_id TEXT,
    issuer_edinet_code TEXT,
    issuer_security_code TEXT,
    issuer_name TEXT,
    ticker TEXT,
    primary_holder_name TEXT,
    group_shares REAL,
    group_holding_pct REAL,
    source_url TEXT NOT NULL,
    raw_index_json TEXT NOT NULL,
    raw_parsed_payload_json TEXT,
    parser_version TEXT NOT NULL,
    status TEXT NOT NULL,
    withdrawn_at TEXT,
    error_message TEXT,
    imported_at INTEGER NOT NULL DEFAULT (unixepoch())
  )`,
  `CREATE INDEX IF NOT EXISTS large_holder_filings_ticker_date_idx
    ON large_holder_filings(ticker, submitted_at DESC)`,
  `CREATE INDEX IF NOT EXISTS large_holder_filings_correction_idx
    ON large_holder_filings(corrected_document_id)`,
  `CREATE TABLE IF NOT EXISTS large_holder_source_documents (
    document_id TEXT PRIMARY KEY REFERENCES large_holder_filings(document_id),
    xbrl_sha256 TEXT NOT NULL,
    xbrl_xml TEXT NOT NULL,
    captured_at INTEGER NOT NULL DEFAULT (unixepoch())
  )`,
  `CREATE TABLE IF NOT EXISTS investor_entities (
    entity_id TEXT PRIMARY KEY,
    canonical_name TEXT NOT NULL,
    display_name TEXT NOT NULL,
    investor_type TEXT NOT NULL DEFAULT 'UNCLASSIFIED',
    investor_class TEXT NOT NULL DEFAULT 'UNCLASSIFIED',
    is_individual INTEGER NOT NULL DEFAULT 0,
    jurisdiction TEXT,
    classification_confidence TEXT NOT NULL DEFAULT 'UNCLASSIFIED',
    manual_override INTEGER NOT NULL DEFAULT 0,
    created_at INTEGER NOT NULL DEFAULT (unixepoch()),
    updated_at INTEGER NOT NULL DEFAULT (unixepoch())
  )`,
  `CREATE TABLE IF NOT EXISTS investor_aliases (
    identity_key TEXT PRIMARY KEY,
    entity_id TEXT NOT NULL REFERENCES investor_entities(entity_id),
    raw_holder_name TEXT NOT NULL,
    raw_holder_address TEXT,
    filer_edinet_code TEXT,
    match_confidence TEXT NOT NULL,
    created_at INTEGER NOT NULL DEFAULT (unixepoch())
  )`,
  `CREATE INDEX IF NOT EXISTS investor_aliases_entity_idx ON investor_aliases(entity_id)`,
  `CREATE TABLE IF NOT EXISTS investor_entity_relations (
    entity_id TEXT NOT NULL REFERENCES investor_entities(entity_id),
    related_entity_id TEXT NOT NULL REFERENCES investor_entities(entity_id),
    status TEXT NOT NULL CHECK (status IN ('CONFIRMED','POSSIBLE','REJECTED')),
    evidence TEXT,
    updated_at INTEGER NOT NULL DEFAULT (unixepoch()),
    PRIMARY KEY (entity_id, related_entity_id)
  )`,
  `CREATE TABLE IF NOT EXISTS large_holder_groups (
    document_id TEXT PRIMARY KEY REFERENCES large_holder_filings(document_id),
    primary_entity_id TEXT REFERENCES investor_entities(entity_id),
    group_shares REAL,
    group_holding_pct REAL,
    participant_count INTEGER NOT NULL
  )`,
  `CREATE TABLE IF NOT EXISTS large_holder_positions (
    document_id TEXT NOT NULL REFERENCES large_holder_filings(document_id),
    holder_key TEXT NOT NULL,
    ticker TEXT NOT NULL,
    entity_id TEXT NOT NULL REFERENCES investor_entities(entity_id),
    holding_group_id TEXT NOT NULL REFERENCES large_holder_groups(document_id),
    holder_role TEXT NOT NULL,
    reported_shares REAL,
    reported_holding_pct REAL,
    previous_reported_shares REAL,
    previous_holding_pct REAL,
    shares_delta REAL,
    holding_pct_delta REAL,
    obligation_date TEXT,
    reference_date TEXT,
    ordinary_share_candidate REAL,
    valuation_eligible_shares REAL,
    reported_equivalent_shares REAL,
    valuation_coverage_pct REAL,
    valuation_basis TEXT NOT NULL,
    valuation_confidence TEXT NOT NULL DEFAULT 'UNCONFIRMED',
    PRIMARY KEY (document_id, holder_key)
  )`,
  `CREATE INDEX IF NOT EXISTS large_holder_positions_entity_ticker_idx
    ON large_holder_positions(entity_id, ticker, obligation_date DESC)`,
  `CREATE TABLE IF NOT EXISTS large_holder_instrument_master (
    instrument_id TEXT NOT NULL,
    source_as_of TEXT NOT NULL,
    issuer_edinet_code TEXT,
    issuer_name TEXT NOT NULL,
    official_security_code TEXT NOT NULL,
    official_security_code_raw TEXT NOT NULL,
    jpx_short_code TEXT,
    isin TEXT,
    instrument_name TEXT NOT NULL,
    instrument_type TEXT NOT NULL,
    security_class TEXT NOT NULL,
    quantity_unit TEXT NOT NULL,
    price_unit TEXT NOT NULL,
    market TEXT NOT NULL,
    listing_status TEXT NOT NULL,
    listed_from TEXT,
    listed_to TEXT,
    price_series_id TEXT,
    source_authority TEXT NOT NULL,
    source_url_or_reference TEXT NOT NULL,
    source_evidence TEXT NOT NULL,
    source_hash TEXT NOT NULL,
    mapping_status TEXT NOT NULL,
    mapping_confidence TEXT NOT NULL,
    created_at INTEGER NOT NULL DEFAULT (unixepoch()),
    updated_at INTEGER NOT NULL DEFAULT (unixepoch()),
    PRIMARY KEY (instrument_id, source_as_of)
  )`,
  `CREATE INDEX IF NOT EXISTS large_holder_instrument_code_date_idx
    ON large_holder_instrument_master(official_security_code_raw, source_as_of)`,
  `CREATE TABLE IF NOT EXISTS large_holder_edinet_code_bridge (
    edinet_code TEXT NOT NULL,
    snapshot_date TEXT NOT NULL,
    security_code TEXT NOT NULL,
    submitter_name TEXT NOT NULL,
    listing_status TEXT NOT NULL,
    source_url TEXT NOT NULL,
    archive_sha256 TEXT NOT NULL,
    source_evidence TEXT NOT NULL,
    source_hash TEXT NOT NULL,
    PRIMARY KEY (edinet_code, snapshot_date)
  )`,
  `CREATE INDEX IF NOT EXISTS large_holder_edinet_bridge_security_idx
    ON large_holder_edinet_code_bridge(security_code, snapshot_date)`,
  `CREATE TABLE IF NOT EXISTS large_holder_issuer_capital_structure (
    issuer_edinet_code TEXT NOT NULL,
    source_document_id TEXT NOT NULL,
    source_submitted_at TEXT NOT NULL,
    reporting_date TEXT NOT NULL,
    source_url TEXT NOT NULL,
    source_evidence TEXT NOT NULL,
    source_hash TEXT NOT NULL,
    classes_json TEXT NOT NULL,
    table_complete INTEGER NOT NULL,
    PRIMARY KEY (issuer_edinet_code, source_document_id)
  )`,
  `CREATE INDEX IF NOT EXISTS large_holder_capital_pit_idx
    ON large_holder_issuer_capital_structure(issuer_edinet_code, source_submitted_at DESC)`,
  `CREATE TABLE IF NOT EXISTS large_holder_price_evidence (
    instrument_id TEXT NOT NULL,
    price_date TEXT NOT NULL,
    official_security_code_raw TEXT NOT NULL,
    raw_close REAL,
    adjusted_close REAL,
    local_close REAL,
    source_authority TEXT NOT NULL,
    source_url_or_reference TEXT NOT NULL,
    source_evidence TEXT,
    source_hash TEXT NOT NULL,
    created_at INTEGER NOT NULL DEFAULT (unixepoch()),
    PRIMARY KEY (instrument_id, price_date)
  )`,
  `CREATE TABLE IF NOT EXISTS large_holder_position_certifications (
    document_id TEXT NOT NULL REFERENCES large_holder_filings(document_id),
    holder_key TEXT NOT NULL,
    instrument_id TEXT,
    mapping_status TEXT NOT NULL,
    price_series_mapping_status TEXT NOT NULL,
    public_status TEXT NOT NULL,
    pit_valuation_ready INTEGER NOT NULL DEFAULT 0,
    quantity_unit TEXT NOT NULL,
    price_unit TEXT NOT NULL,
    eligible_units REAL,
    price_date TEXT,
    price_per_unit REAL,
    estimated_value_yen REAL,
    current_value_yen REAL,
    certification_method TEXT NOT NULL DEFAULT 'OTHER',
    certification_strength TEXT NOT NULL DEFAULT 'NONE',
    certification_evidence_json TEXT NOT NULL DEFAULT '[]',
    evidence_json TEXT NOT NULL,
    assessed_at INTEGER NOT NULL DEFAULT (unixepoch()),
    PRIMARY KEY (document_id, holder_key)
  )`,
  `CREATE TABLE IF NOT EXISTS investor_identity_reviews (
    identity_key TEXT PRIMARY KEY,
    reason TEXT NOT NULL,
    status TEXT NOT NULL DEFAULT 'PENDING',
    created_at INTEGER NOT NULL DEFAULT (unixepoch())
  )`,
  `CREATE TABLE IF NOT EXISTS stock_external_data_status (
    ticker TEXT NOT NULL,
    dataset TEXT NOT NULL,
    status TEXT NOT NULL,
    source_date TEXT,
    message TEXT,
    attempted_at INTEGER NOT NULL DEFAULT (unixepoch()),
    PRIMARY KEY (ticker, dataset)
  )`,
  // ─── Phase 4: 空売り残高 ───
  `CREATE TABLE IF NOT EXISTS short_selling_positions (
    ticker TEXT NOT NULL,
    date TEXT NOT NULL,
    short_ratio REAL,
    reporter TEXT NOT NULL DEFAULT '',
    imported_at INTEGER NOT NULL DEFAULT (unixepoch()),
    PRIMARY KEY (ticker, date, reporter)
  )`,
  `CREATE INDEX IF NOT EXISTS ssp_date_idx ON short_selling_positions(date)`,
  `CREATE INDEX IF NOT EXISTS ssp_ticker_date_idx ON short_selling_positions(ticker, date)`,
  // ─── Phase 4: 決算発表カレンダー ───
  `CREATE TABLE IF NOT EXISTS earnings_calendar (
    ticker TEXT NOT NULL,
    announce_date TEXT NOT NULL,
    fiscal_period TEXT,
    company_name TEXT,
    sector_name TEXT,
    market_segment TEXT,
    source TEXT,
    source_url TEXT,
    scheduled_time TEXT,
    scheduled_time_kind TEXT,
    scheduled_time_source TEXT,
    scheduled_time_source_url TEXT,
    predicted_time TEXT,
    prediction_confidence TEXT,
    prediction_sample_count INTEGER,
    prediction_mode_count INTEGER,
    actual_disclosed_date TEXT,
    actual_disclosed_time TEXT,
    actual_disclosed_at TEXT,
    actual_source TEXT,
    actual_source_url TEXT,
    time_bucket TEXT,
    time_updated_at INTEGER,
    imported_at INTEGER NOT NULL DEFAULT (unixepoch()),
    PRIMARY KEY (ticker, announce_date)
  )`,
  `CREATE INDEX IF NOT EXISTS earn_date_idx ON earnings_calendar(announce_date)`,
  `CREATE INDEX IF NOT EXISTS earn_import_date_ticker_idx
    ON earnings_calendar(imported_at, announce_date, ticker)`,
  // ─── Phase 4: 6 桁パターンコード検索用 expression index ───
  `CREATE INDEX IF NOT EXISTS idx_ds_pattern_code ON daily_snapshots(
    (CAST(daily_a_stage AS TEXT) || CAST(daily_b_stage AS TEXT) || CAST(weekly_a_stage AS TEXT) || CAST(weekly_b_stage AS TEXT) || CAST(monthly_a_stage AS TEXT) || CAST(monthly_b_stage AS TEXT))
  )`,
  // ─── Phase 5: Backtest / ML-RL readiness ───
  `CREATE TABLE IF NOT EXISTS weekly_ohlcv (
    ticker TEXT NOT NULL,
    date TEXT NOT NULL,
    week_start_date TEXT NOT NULL,
    week_end_date TEXT NOT NULL,
    open REAL NOT NULL,
    high REAL NOT NULL,
    low REAL NOT NULL,
    close REAL NOT NULL,
    volume INTEGER NOT NULL,
    ma_5 REAL,
    ma_13 REAL,
    ma_25 REAL,
    ma_50 REAL,
    ma_100 REAL,
    computed_at INTEGER NOT NULL DEFAULT (unixepoch()),
    PRIMARY KEY (ticker, date)
  )`,
  `CREATE INDEX IF NOT EXISTS weekly_ohlcv_date_idx ON weekly_ohlcv(date)`,
  `CREATE TABLE IF NOT EXISTS technical_signals (
    ticker TEXT NOT NULL,
    date TEXT NOT NULL,
    timescale TEXT NOT NULL,
    ma_period INTEGER NOT NULL DEFAULT 0,
    signal_code TEXT NOT NULL,
    signal_strength TEXT NOT NULL,
    direction TEXT NOT NULL,
    label TEXT NOT NULL,
    score_component REAL,
    value_json TEXT,
    computed_at INTEGER NOT NULL DEFAULT (unixepoch()),
    PRIMARY KEY (ticker, date, timescale, ma_period, signal_code)
  )`,
  `CREATE INDEX IF NOT EXISTS tech_signal_date_idx ON technical_signals(date, signal_code)`,
  `CREATE INDEX IF NOT EXISTS tech_signal_code_idx ON technical_signals(signal_code, date)`,
  `CREATE INDEX IF NOT EXISTS tech_signal_date_ticker_idx ON technical_signals(date, ticker)`,
  `CREATE INDEX IF NOT EXISTS tech_signal_ticker_code_date_idx ON technical_signals(ticker, signal_code, date)`,
  `CREATE TABLE IF NOT EXISTS forward_extrema (
    ticker TEXT NOT NULL,
    date TEXT NOT NULL,
    horizon_days INTEGER NOT NULL,
    return_pct REAL,
    end_date TEXT,
    max_return_pct REAL,
    max_return_date TEXT,
    days_to_max INTEGER,
    min_return_pct REAL,
    min_return_date TEXT,
    days_to_min INTEGER,
    hit_10 INTEGER NOT NULL DEFAULT 0,
    hit_20 INTEGER NOT NULL DEFAULT 0,
    hit_40 INTEGER NOT NULL DEFAULT 0,
    days_to_10 INTEGER,
    days_to_20 INTEGER,
    days_to_40 INTEGER,
    computed_at INTEGER NOT NULL DEFAULT (unixepoch()),
    PRIMARY KEY (ticker, date, horizon_days)
  )`,
  `CREATE INDEX IF NOT EXISTS fext_date_horizon_idx ON forward_extrema(date, horizon_days)`,
  `CREATE INDEX IF NOT EXISTS fext_date_horizon_ticker_idx ON forward_extrema(date, horizon_days, ticker)`,
  `CREATE INDEX IF NOT EXISTS fext_date_horizon_max_return_idx ON forward_extrema(date, horizon_days, max_return_pct DESC)`,
  `CREATE INDEX IF NOT EXISTS fext_horizon_date_idx ON forward_extrema(horizon_days, date)`,
  `CREATE INDEX IF NOT EXISTS fext_horizon_max_idx ON forward_extrema(horizon_days, max_return_pct)`,
  `CREATE TABLE IF NOT EXISTS forward_extrema_date_coverage (
    horizon_days INTEGER NOT NULL,
    date TEXT NOT NULL,
    updated_at INTEGER NOT NULL DEFAULT (unixepoch()),
    PRIMARY KEY (horizon_days, date)
  )`,
  `CREATE TABLE IF NOT EXISTS model_features (
    ticker TEXT NOT NULL,
    date TEXT NOT NULL,
    pattern_code TEXT,
    daily_a_stage INTEGER,
    daily_b_stage INTEGER,
    weekly_a_stage INTEGER,
    weekly_b_stage INTEGER,
    monthly_a_stage INTEGER,
    monthly_b_stage INTEGER,
    close REAL,
    volume INTEGER,
    volume_ratio_20 REAL,
    range_pct REAL,
    atr20_pct REAL,
    ma5_pos_pct REAL,
    ma25_pos_pct REAL,
    ma75_pos_pct REAL,
    ma_spread_pct REAL,
    rel_strength_20 REAL,
    signal_codes TEXT,
    feature_json TEXT,
    computed_at INTEGER NOT NULL DEFAULT (unixepoch()),
    PRIMARY KEY (ticker, date)
  )`,
  `CREATE INDEX IF NOT EXISTS model_features_date_idx ON model_features(date)`,
  `CREATE INDEX IF NOT EXISTS model_features_pattern_idx ON model_features(pattern_code, date)`,
  `CREATE INDEX IF NOT EXISTS model_features_ticker_date_idx ON model_features(ticker, date)`,
  `CREATE TABLE IF NOT EXISTS model_labels (
    ticker TEXT NOT NULL,
    date TEXT NOT NULL,
    horizon_days INTEGER NOT NULL,
    return_pct REAL,
    max_return_pct REAL,
    min_return_pct REAL,
    days_to_max INTEGER,
    hit_10 INTEGER NOT NULL DEFAULT 0,
    hit_20 INTEGER NOT NULL DEFAULT 0,
    hit_40 INTEGER NOT NULL DEFAULT 0,
    reward_score REAL,
    label_json TEXT,
    computed_at INTEGER NOT NULL DEFAULT (unixepoch()),
    PRIMARY KEY (ticker, date, horizon_days)
  )`,
  `CREATE INDEX IF NOT EXISTS model_labels_date_idx ON model_labels(date, horizon_days)`,
  `CREATE TABLE IF NOT EXISTS signal_stats (
    signal_code TEXT NOT NULL,
    pattern_code TEXT NOT NULL,
    horizon_days INTEGER NOT NULL,
    count INTEGER NOT NULL,
    hit_10_rate REAL,
    hit_20_rate REAL,
    hit_40_rate REAL,
    max_return_p25 REAL,
    max_return_p50 REAL,
    max_return_p75 REAL,
    return_p50 REAL,
    min_return_p50 REAL,
    days_to_max_p50 REAL,
    computed_at INTEGER NOT NULL DEFAULT (unixepoch()),
    PRIMARY KEY (signal_code, pattern_code, horizon_days)
  )`,
  `CREATE INDEX IF NOT EXISTS signal_stats_code_idx ON signal_stats(signal_code, horizon_days, count)`,
  `CREATE TABLE IF NOT EXISTS signal_return_stats (
    signal_code TEXT NOT NULL,
    horizon_days INTEGER NOT NULL,
    count INTEGER NOT NULL,
    up_rate REAL,
    down_rate REAL,
    median_return_pct REAL,
    avg_return_pct REAL,
    computed_at INTEGER NOT NULL DEFAULT (unixepoch()),
    PRIMARY KEY (signal_code, horizon_days)
  )`,
  `CREATE INDEX IF NOT EXISTS signal_return_stats_code_idx ON signal_return_stats(signal_code, horizon_days, count)`,
  `CREATE TABLE IF NOT EXISTS serving_latest_signals (
    date TEXT NOT NULL,
    ticker TEXT NOT NULL,
    rank INTEGER NOT NULL,
    score REAL NOT NULL,
    signal_codes TEXT NOT NULL,
    summary_json TEXT NOT NULL,
    computed_at INTEGER NOT NULL DEFAULT (unixepoch()),
    PRIMARY KEY (date, ticker)
  )`,
  `CREATE INDEX IF NOT EXISTS serving_latest_rank_idx ON serving_latest_signals(date, rank)`,
  `CREATE TABLE IF NOT EXISTS serving_signal_stats (
    signal_code TEXT NOT NULL,
    pattern_code TEXT NOT NULL,
    horizon_days INTEGER NOT NULL,
    payload_json TEXT NOT NULL,
    computed_at INTEGER NOT NULL DEFAULT (unixepoch()),
    PRIMARY KEY (signal_code, pattern_code, horizon_days)
  )`,
  `CREATE TABLE IF NOT EXISTS serving_backtest_dates (
    date TEXT PRIMARY KEY,
    total_tickers INTEGER NOT NULL,
    signal_tickers INTEGER NOT NULL,
    computed_at INTEGER NOT NULL DEFAULT (unixepoch())
  )`,
  `CREATE TABLE IF NOT EXISTS serving_backtest_summaries (
    date TEXT NOT NULL,
    horizon_days INTEGER NOT NULL,
    condition_key TEXT NOT NULL,
    payload_json TEXT NOT NULL,
    computed_at INTEGER NOT NULL DEFAULT (unixepoch()),
    PRIMARY KEY (date, horizon_days, condition_key)
  )`,
  `CREATE INDEX IF NOT EXISTS serving_summary_date_idx ON serving_backtest_summaries(date, horizon_days)`,
  `CREATE TABLE IF NOT EXISTS serving_similar_cases (
    source_ticker TEXT NOT NULL,
    source_date TEXT NOT NULL,
    rank INTEGER NOT NULL,
    similar_ticker TEXT NOT NULL,
    similar_date TEXT NOT NULL,
    similarity_score REAL NOT NULL,
    payload_json TEXT NOT NULL,
    computed_at INTEGER NOT NULL DEFAULT (unixepoch()),
    PRIMARY KEY (source_ticker, source_date, rank)
  )`,
  `CREATE TABLE IF NOT EXISTS serving_backtest_results (
    date TEXT NOT NULL,
    horizon_days INTEGER NOT NULL,
    ticker TEXT NOT NULL,
    name TEXT,
    sector_large TEXT,
    sector_small TEXT,
    market_segment TEXT,
    pattern_code TEXT,
    daily_a_stage INTEGER,
    daily_b_stage INTEGER,
    weekly_a_stage INTEGER,
    weekly_b_stage INTEGER,
    monthly_a_stage INTEGER,
    monthly_b_stage INTEGER,
    open REAL,
    high REAL,
    low REAL,
    close REAL,
    volume INTEGER,
    volume_ratio_20 REAL,
    range_pct REAL,
    atr20_pct REAL,
    ma5_pos_pct REAL,
    ma25_pos_pct REAL,
    ma75_pos_pct REAL,
    signal_codes TEXT,
    return_pct REAL,
    max_return_pct REAL,
    max_return_date TEXT,
    days_to_max INTEGER,
    min_return_pct REAL,
    min_return_date TEXT,
    days_to_min INTEGER,
    hit_10 INTEGER,
    hit_20 INTEGER,
    hit_40 INTEGER,
    computed_at INTEGER NOT NULL DEFAULT (unixepoch()),
    PRIMARY KEY (date, horizon_days, ticker)
  )`,
  `CREATE INDEX IF NOT EXISTS serving_backtest_results_sort_idx ON serving_backtest_results(date, horizon_days, max_return_pct)`,
  `CREATE INDEX IF NOT EXISTS serving_backtest_results_horizon_date_idx ON serving_backtest_results(horizon_days, date DESC)`,
  `CREATE INDEX IF NOT EXISTS serving_backtest_results_ticker_idx ON serving_backtest_results(ticker, date)`,
  `CREATE TABLE IF NOT EXISTS serving_backtest_details (
    date TEXT NOT NULL,
    horizon_days INTEGER NOT NULL,
    ticker TEXT NOT NULL,
    detail_json TEXT NOT NULL,
    computed_at INTEGER NOT NULL DEFAULT (unixepoch()),
    PRIMARY KEY (date, horizon_days, ticker)
  )`,
  `CREATE TABLE IF NOT EXISTS serving_signal_evidence (
    ticker TEXT NOT NULL,
    date TEXT NOT NULL,
    signal_code TEXT NOT NULL,
    label TEXT,
    reason_json TEXT NOT NULL,
    computed_at INTEGER NOT NULL DEFAULT (unixepoch()),
    PRIMARY KEY (ticker, date, signal_code)
  )`,
  `CREATE INDEX IF NOT EXISTS serving_signal_evidence_date_idx ON serving_signal_evidence(date, signal_code)`,
  `CREATE TABLE IF NOT EXISTS serving_stock_metrics (
    ticker TEXT PRIMARY KEY,
    as_of_date TEXT NOT NULL,
    payload_json TEXT NOT NULL,
    computed_at INTEGER NOT NULL DEFAULT (unixepoch())
  )`,
  `CREATE TABLE IF NOT EXISTS serving_stock_move_periods (
    ticker TEXT NOT NULL,
    direction TEXT NOT NULL,
    rank INTEGER NOT NULL,
    start_date TEXT NOT NULL,
    end_date TEXT NOT NULL,
    return_pct REAL NOT NULL,
    trading_days INTEGER NOT NULL,
    stage_path_json TEXT NOT NULL,
    payload_json TEXT NOT NULL,
    computed_at INTEGER NOT NULL DEFAULT (unixepoch()),
    PRIMARY KEY (ticker, direction, rank)
  )`,
  `CREATE INDEX IF NOT EXISTS serving_stock_move_ticker_idx ON serving_stock_move_periods(ticker, direction, rank)`,
  `CREATE TABLE IF NOT EXISTS ml_feature_vectors (
    ticker TEXT NOT NULL,
    date TEXT NOT NULL,
    stage_code TEXT,
    feature_json TEXT NOT NULL,
    vector_json TEXT NOT NULL,
    computed_at INTEGER NOT NULL DEFAULT (unixepoch()),
    PRIMARY KEY (ticker, date)
  )`,
  `CREATE INDEX IF NOT EXISTS ml_feature_vectors_date_idx ON ml_feature_vectors(date)`,
  `CREATE TABLE IF NOT EXISTS ml_feature_vectors_v2 (
    ticker TEXT NOT NULL,
    date TEXT NOT NULL,
    feature_set TEXT NOT NULL,
    version INTEGER NOT NULL DEFAULT 1,
    stage_code TEXT,
    feature_json TEXT NOT NULL,
    vector_json TEXT NOT NULL,
    computed_at INTEGER NOT NULL DEFAULT (unixepoch()),
    PRIMARY KEY (ticker, date, feature_set)
  )`,
  `CREATE INDEX IF NOT EXISTS ml_feature_vectors_v2_date_idx ON ml_feature_vectors_v2(feature_set, date)`,
  `CREATE INDEX IF NOT EXISTS ml_feature_vectors_v2_date_only_idx ON ml_feature_vectors_v2(date)`,
  `CREATE INDEX IF NOT EXISTS ml_feature_vectors_v2_ticker_date_idx ON ml_feature_vectors_v2(ticker, date)`,
  `CREATE INDEX IF NOT EXISTS ml_feature_vectors_v2_feature_ticker_date_idx ON ml_feature_vectors_v2(feature_set, ticker, date)`,
  `CREATE TABLE IF NOT EXISTS ml_training_labels (
    ticker TEXT NOT NULL,
    date TEXT NOT NULL,
    horizon_days INTEGER NOT NULL,
    return_pct REAL,
    max_return_pct REAL,
    min_return_pct REAL,
    up_label INTEGER NOT NULL DEFAULT 0,
    down_label INTEGER NOT NULL DEFAULT 0,
    reward_score REAL,
    label_json TEXT,
    computed_at INTEGER NOT NULL DEFAULT (unixepoch()),
    PRIMARY KEY (ticker, date, horizon_days)
  )`,
  `CREATE INDEX IF NOT EXISTS ml_training_labels_date_idx ON ml_training_labels(date, horizon_days)`,
  `CREATE TABLE IF NOT EXISTS ml_short_labels (
    ticker TEXT NOT NULL,
    date TEXT NOT NULL,
    horizon_days INTEGER NOT NULL,
    return_pct REAL,
    max_return_pct REAL,
    min_return_pct REAL,
    up_label INTEGER NOT NULL DEFAULT 0,
    down_label INTEGER NOT NULL DEFAULT 0,
    wait_label INTEGER NOT NULL DEFAULT 0,
    reward_long REAL,
    reward_short REAL,
    reward_wait REAL,
    label_json TEXT NOT NULL,
    computed_at INTEGER NOT NULL DEFAULT (unixepoch()),
    PRIMARY KEY (ticker, date, horizon_days)
  )`,
  `CREATE INDEX IF NOT EXISTS ml_short_labels_date_idx ON ml_short_labels(date, horizon_days)`,
  `CREATE INDEX IF NOT EXISTS ml_short_labels_horizon_idx ON ml_short_labels(horizon_days, date)`,
  `CREATE TABLE IF NOT EXISTS ml_models (
    model_name TEXT PRIMARY KEY,
    model_type TEXT NOT NULL,
    direction TEXT NOT NULL,
    horizon_days INTEGER NOT NULL,
    feature_names_json TEXT NOT NULL,
    weights_json TEXT NOT NULL,
    intercept REAL NOT NULL DEFAULT 0,
    metrics_json TEXT NOT NULL,
    trained_at INTEGER NOT NULL DEFAULT (unixepoch())
  )`,
  `CREATE INDEX IF NOT EXISTS ml_models_latest_idx ON ml_models(horizon_days, direction, trained_at)`,
  `CREATE TABLE IF NOT EXISTS ml_prediction_runs (
    run_id TEXT PRIMARY KEY,
    as_of_date TEXT NOT NULL,
    horizon_days INTEGER NOT NULL,
    direction TEXT NOT NULL,
    model_name TEXT,
    model_type TEXT NOT NULL,
    prediction_count INTEGER NOT NULL DEFAULT 0,
    source TEXT NOT NULL DEFAULT 'batch',
    status TEXT NOT NULL DEFAULT 'success',
    created_at INTEGER NOT NULL DEFAULT (unixepoch())
  )`,
  `CREATE INDEX IF NOT EXISTS ml_prediction_runs_date_idx ON ml_prediction_runs(as_of_date, horizon_days, direction)`,
  `CREATE TABLE IF NOT EXISTS ml_predictions (
    as_of_date TEXT NOT NULL,
    horizon_days INTEGER NOT NULL,
    direction TEXT NOT NULL,
    ticker TEXT NOT NULL,
    run_id TEXT NOT NULL,
    rank INTEGER NOT NULL,
    score REAL NOT NULL,
    model_name TEXT,
    feature_json TEXT NOT NULL,
    reason_json TEXT NOT NULL,
    explanation_json TEXT NOT NULL,
    created_at INTEGER NOT NULL DEFAULT (unixepoch()),
    PRIMARY KEY (as_of_date, horizon_days, direction, ticker)
  )`,
  `CREATE INDEX IF NOT EXISTS ml_predictions_rank_idx ON ml_predictions(as_of_date, horizon_days, direction, rank)`,
  `CREATE INDEX IF NOT EXISTS ml_predictions_ticker_idx ON ml_predictions(ticker, as_of_date)`,
  `CREATE TABLE IF NOT EXISTS ml_prediction_outcomes (
    as_of_date TEXT NOT NULL,
    horizon_days INTEGER NOT NULL,
    direction TEXT NOT NULL,
    ticker TEXT NOT NULL,
    score REAL NOT NULL,
    rank INTEGER NOT NULL,
    return_pct REAL,
    max_return_pct REAL,
    min_return_pct REAL,
    hit_label INTEGER NOT NULL DEFAULT 0,
    miss_label INTEGER NOT NULL DEFAULT 0,
    outcome_json TEXT NOT NULL,
    evaluated_at INTEGER NOT NULL DEFAULT (unixepoch()),
    PRIMARY KEY (as_of_date, horizon_days, direction, ticker)
  )`,
  `CREATE INDEX IF NOT EXISTS ml_prediction_outcomes_date_idx ON ml_prediction_outcomes(as_of_date, horizon_days, direction)`,
  `CREATE INDEX IF NOT EXISTS ml_prediction_outcomes_ticker_idx ON ml_prediction_outcomes(ticker, as_of_date)`,
  `CREATE TABLE IF NOT EXISTS ml_model_evaluations (
    evaluation_id TEXT PRIMARY KEY,
    model_name TEXT,
    model_type TEXT NOT NULL,
    direction TEXT NOT NULL,
    horizon_days INTEGER NOT NULL,
    evaluation_date TEXT NOT NULL,
    train_start_date TEXT,
    train_end_date TEXT,
    validation_start_date TEXT,
    validation_end_date TEXT,
    sample_count INTEGER NOT NULL DEFAULT 0,
    precision_at_20 REAL,
    precision_at_50 REAL,
    precision_at_80 REAL,
    hit_rate REAL,
    median_return_pct REAL,
    avg_return_pct REAL,
    max_drawdown_pct REAL,
    metrics_json TEXT NOT NULL,
    created_at INTEGER NOT NULL DEFAULT (unixepoch())
  )`,
  `CREATE INDEX IF NOT EXISTS ml_model_evaluations_latest_idx ON ml_model_evaluations(evaluation_date, direction, horizon_days)`,
  `CREATE TABLE IF NOT EXISTS rl_training_states (
    ticker TEXT NOT NULL,
    date TEXT NOT NULL,
    horizon_days INTEGER NOT NULL,
    action TEXT NOT NULL,
    state_json TEXT NOT NULL,
    reward REAL,
    next_state_json TEXT,
    computed_at INTEGER NOT NULL DEFAULT (unixepoch()),
    PRIMARY KEY (ticker, date, horizon_days, action)
  )`,
  `CREATE INDEX IF NOT EXISTS rl_training_states_date_idx ON rl_training_states(date, horizon_days)`,
  `CREATE TABLE IF NOT EXISTS rl_training_states_v2 (
    ticker TEXT NOT NULL,
    date TEXT NOT NULL,
    horizon_days INTEGER NOT NULL,
    action TEXT NOT NULL,
    state_json TEXT NOT NULL,
    reward REAL,
    next_state_json TEXT,
    computed_at INTEGER NOT NULL DEFAULT (unixepoch()),
    PRIMARY KEY (ticker, date, horizon_days, action)
  )`,
  `CREATE INDEX IF NOT EXISTS rl_training_states_v2_date_idx ON rl_training_states_v2(date, horizon_days)`,
  `CREATE TABLE IF NOT EXISTS ml_rl_policy_evaluations (
    evaluation_id TEXT PRIMARY KEY,
    policy_name TEXT NOT NULL,
    policy_type TEXT NOT NULL,
    horizon_days INTEGER NOT NULL,
    evaluation_date TEXT NOT NULL,
    start_date TEXT,
    end_date TEXT,
    sample_count INTEGER NOT NULL DEFAULT 0,
    long_count INTEGER NOT NULL DEFAULT 0,
    short_count INTEGER NOT NULL DEFAULT 0,
    wait_count INTEGER NOT NULL DEFAULT 0,
    win_rate REAL,
    oracle_match_rate REAL,
    avg_reward REAL,
    median_reward REAL,
    avg_return_pct REAL,
    max_drawdown_pct REAL,
    action_breakdown_json TEXT NOT NULL,
    metrics_json TEXT NOT NULL,
    created_at INTEGER NOT NULL DEFAULT (unixepoch())
  )`,
  `CREATE INDEX IF NOT EXISTS ml_rl_policy_evaluations_latest_idx ON ml_rl_policy_evaluations(evaluation_date, horizon_days, policy_name)`,
  `CREATE TABLE IF NOT EXISTS ml_physics_status_evaluations (
    evaluation_id TEXT PRIMARY KEY,
    evaluation_date TEXT NOT NULL,
    feature_set TEXT NOT NULL,
    status_label TEXT NOT NULL,
    target_direction TEXT NOT NULL,
    horizon_days INTEGER NOT NULL,
    start_date TEXT,
    end_date TEXT,
    sample_count INTEGER NOT NULL DEFAULT 0,
    hit_count INTEGER NOT NULL DEFAULT 0,
    adverse_count INTEGER NOT NULL DEFAULT 0,
    hit_rate REAL,
    base_rate REAL,
    lift REAL,
    confidence_score REAL,
    median_return_pct REAL,
    avg_return_pct REAL,
    avg_max_return_pct REAL,
    avg_min_return_pct REAL,
    adverse_rate REAL,
    metrics_json TEXT NOT NULL,
    created_at INTEGER NOT NULL DEFAULT (unixepoch())
  )`,
  `CREATE INDEX IF NOT EXISTS ml_physics_status_evaluations_latest_idx ON ml_physics_status_evaluations(evaluation_date, horizon_days, status_label)`,
  `CREATE INDEX IF NOT EXISTS ml_physics_status_evaluations_status_idx ON ml_physics_status_evaluations(status_label, horizon_days, evaluation_date)`,
  `CREATE INDEX IF NOT EXISTS ml_physics_status_evaluations_lookup_idx
     ON ml_physics_status_evaluations(feature_set, status_label, horizon_days, evaluation_date)`,
  `CREATE TABLE IF NOT EXISTS serving_ml_candidates (
    as_of_date TEXT NOT NULL,
    direction TEXT NOT NULL,
    rank INTEGER NOT NULL,
    ticker TEXT NOT NULL,
    name TEXT,
    sector_large TEXT,
    candidate_score REAL NOT NULL,
    model_name TEXT,
    feature_json TEXT NOT NULL,
    reason_json TEXT NOT NULL,
    explanation_json TEXT NOT NULL,
    computed_at INTEGER NOT NULL DEFAULT (unixepoch()),
    PRIMARY KEY (as_of_date, direction, ticker)
  )`,
  `CREATE INDEX IF NOT EXISTS serving_ml_candidates_rank_idx ON serving_ml_candidates(as_of_date, direction, rank)`,
  `CREATE TABLE IF NOT EXISTS serving_ml_physics_candidates (
    as_of_date TEXT NOT NULL,
    direction TEXT NOT NULL,
    horizon_days INTEGER NOT NULL,
    rank INTEGER NOT NULL,
    ticker TEXT NOT NULL,
    name TEXT,
    sector_large TEXT,
    candidate_score REAL NOT NULL,
    model_name TEXT,
    feature_json TEXT NOT NULL,
    reason_json TEXT NOT NULL,
    explanation_json TEXT NOT NULL,
    computed_at INTEGER NOT NULL DEFAULT (unixepoch()),
    PRIMARY KEY (as_of_date, direction, horizon_days, ticker)
  )`,
  `CREATE INDEX IF NOT EXISTS serving_ml_physics_candidates_rank_idx ON serving_ml_physics_candidates(as_of_date, horizon_days, direction, rank)`,
  `CREATE INDEX IF NOT EXISTS serving_ml_physics_candidates_ticker_idx ON serving_ml_physics_candidates(ticker, as_of_date)`,
  `CREATE INDEX IF NOT EXISTS serving_ml_physics_candidates_horizon_date_idx
     ON serving_ml_physics_candidates(horizon_days, as_of_date)`,
  `CREATE INDEX IF NOT EXISTS serving_ml_physics_candidates_horizon_ticker_date_idx
     ON serving_ml_physics_candidates(horizon_days, ticker, as_of_date)`,
  `CREATE TABLE IF NOT EXISTS serving_current_similars (
    as_of_date TEXT NOT NULL,
    base_ticker TEXT NOT NULL,
    rank INTEGER NOT NULL,
    similar_ticker TEXT NOT NULL,
    similarity_score REAL NOT NULL,
    base_direction TEXT,
    similar_direction TEXT,
    payload_json TEXT NOT NULL,
    reason_json TEXT NOT NULL,
    computed_at INTEGER NOT NULL DEFAULT (unixepoch()),
    PRIMARY KEY (as_of_date, base_ticker, rank)
  )`,
  `CREATE INDEX IF NOT EXISTS serving_current_similars_base_idx ON serving_current_similars(as_of_date, base_ticker)`,
  `CREATE INDEX IF NOT EXISTS serving_current_similars_score_idx ON serving_current_similars(as_of_date, similarity_score)`,
  `CREATE INDEX IF NOT EXISTS serving_current_similars_similar_idx ON serving_current_similars(as_of_date, similar_ticker)`,
  `CREATE TABLE IF NOT EXISTS serving_ml_sector_rankings (
    as_of_date TEXT NOT NULL,
    sector_type TEXT NOT NULL,
    sector_name TEXT NOT NULL,
    direction TEXT NOT NULL,
    candidate_count INTEGER NOT NULL DEFAULT 0,
    avg_score REAL,
    representative_tickers_json TEXT NOT NULL,
    computed_at INTEGER NOT NULL DEFAULT (unixepoch()),
    PRIMARY KEY (as_of_date, sector_type, sector_name, direction)
  )`,
  `CREATE INDEX IF NOT EXISTS serving_ml_sector_rankings_rank_idx ON serving_ml_sector_rankings(as_of_date, sector_type, direction, candidate_count)`,
  `CREATE TABLE IF NOT EXISTS serving_ml_performance (
    as_of_date TEXT NOT NULL,
    direction TEXT NOT NULL,
    horizon_days INTEGER NOT NULL,
    sector_type TEXT NOT NULL DEFAULT 'all',
    sector_name TEXT NOT NULL DEFAULT 'ALL',
    sample_count INTEGER NOT NULL DEFAULT 0,
    up_rate REAL,
    down_rate REAL,
    median_return_pct REAL,
    avg_return_pct REAL,
    payload_json TEXT NOT NULL,
    computed_at INTEGER NOT NULL DEFAULT (unixepoch()),
    PRIMARY KEY (as_of_date, direction, horizon_days, sector_type, sector_name)
  )`,
  `CREATE INDEX IF NOT EXISTS serving_ml_performance_rank_idx ON serving_ml_performance(as_of_date, direction, horizon_days, sample_count)`,
  `CREATE TABLE IF NOT EXISTS ml_market_context_features (
    date TEXT PRIMARY KEY,
    market_return_5 REAL,
    market_return_20 REAL,
    market_above_sma25_rate REAL,
    market_above_sma75_rate REAL,
    advancers_rate_5 REAL,
    sample_count INTEGER NOT NULL DEFAULT 0,
    payload_json TEXT NOT NULL,
    computed_at INTEGER NOT NULL DEFAULT (unixepoch())
  )`,
  `CREATE TABLE IF NOT EXISTS ml_sector_context_features (
    date TEXT NOT NULL,
    sector_type TEXT NOT NULL,
    sector_name TEXT NOT NULL,
    return_5 REAL,
    return_20 REAL,
    above_sma25_rate REAL,
    rank_pct REAL,
    sample_count INTEGER NOT NULL DEFAULT 0,
    payload_json TEXT NOT NULL,
    computed_at INTEGER NOT NULL DEFAULT (unixepoch()),
    PRIMARY KEY (date, sector_type, sector_name)
  )`,
  `CREATE INDEX IF NOT EXISTS ml_sector_context_features_rank_idx ON ml_sector_context_features(date, sector_type, rank_pct)`,
  `CREATE TABLE IF NOT EXISTS ml_similarity_evaluations (
    as_of_date TEXT NOT NULL,
    horizon_days INTEGER NOT NULL,
    source TEXT NOT NULL,
    base_count INTEGER NOT NULL DEFAULT 0,
    pair_count INTEGER NOT NULL DEFAULT 0,
    up_rate REAL,
    down_rate REAL,
    median_return_pct REAL,
    avg_return_pct REAL,
    max_drawdown_pct REAL,
    payload_json TEXT NOT NULL,
    computed_at INTEGER NOT NULL DEFAULT (unixepoch()),
    PRIMARY KEY (as_of_date, horizon_days, source)
  )`,
  `CREATE TABLE IF NOT EXISTS ml_feature_health_checks (
    check_date TEXT NOT NULL,
    check_key TEXT NOT NULL,
    status TEXT NOT NULL,
    expected_date TEXT,
    actual_date TEXT,
    expected_count INTEGER,
    actual_count INTEGER,
    payload_json TEXT NOT NULL,
    computed_at INTEGER NOT NULL DEFAULT (unixepoch()),
    PRIMARY KEY (check_date, check_key)
  )`,
  `CREATE TABLE IF NOT EXISTS ml_pipeline_generations (
    market TEXT NOT NULL,
    pipeline TEXT NOT NULL,
    generation_version TEXT NOT NULL,
    status TEXT NOT NULL,
    baseline_source_date TEXT NOT NULL,
    baseline_start_date TEXT,
    last_delta_source_date TEXT NOT NULL,
    price_basis TEXT,
    feature_set TEXT NOT NULL,
    baseline_completed_at INTEGER NOT NULL,
    last_delta_completed_at INTEGER NOT NULL,
    payload_json TEXT NOT NULL DEFAULT '{}',
    updated_at INTEGER NOT NULL DEFAULT (unixepoch()),
    PRIMARY KEY (market, pipeline)
  )`,
  `CREATE TABLE IF NOT EXISTS turso_sync_runs (
    run_id TEXT PRIMARY KEY,
    mode TEXT NOT NULL,
    status TEXT NOT NULL,
    dry_run INTEGER NOT NULL DEFAULT 0,
    started_at INTEGER NOT NULL DEFAULT (unixepoch()),
    finished_at INTEGER,
    summary_json TEXT,
    error_message TEXT
  )`,
  `CREATE TABLE IF NOT EXISTS turso_sync_partitions (
    mode TEXT NOT NULL,
    table_name TEXT NOT NULL,
    partition_key TEXT NOT NULL,
    range_start TEXT,
    range_end TEXT,
    row_count INTEGER NOT NULL DEFAULT 0,
    status TEXT NOT NULL,
    run_id TEXT,
    synced_at INTEGER,
    error_message TEXT,
    PRIMARY KEY (mode, table_name, partition_key)
  )`,
  `CREATE INDEX IF NOT EXISTS turso_sync_partitions_status_idx ON turso_sync_partitions(mode, status, table_name)`,
  `CREATE TABLE IF NOT EXISTS fx_rates_daily (
    pair TEXT NOT NULL,
    date TEXT NOT NULL,
    rate REAL NOT NULL,
    source TEXT NOT NULL DEFAULT 'csv',
    imported_at INTEGER NOT NULL DEFAULT (unixepoch()),
    PRIMARY KEY (pair, date)
  )`,
  `CREATE INDEX IF NOT EXISTS fx_rates_daily_date_idx ON fx_rates_daily(date)`,
  `CREATE TABLE IF NOT EXISTS custom_charts (
    id TEXT PRIMARY KEY,
    name TEXT NOT NULL,
    formula TEXT NOT NULL,
    formula_ast_json TEXT NOT NULL,
    mode TEXT NOT NULL DEFAULT 'valuation',
    base_date TEXT,
    display_currency TEXT NOT NULL DEFAULT 'LOCAL',
    missing_policy TEXT NOT NULL DEFAULT 'intersection',
    indicator_config_json TEXT NOT NULL,
    favorite INTEGER NOT NULL DEFAULT 0,
    sort_order INTEGER NOT NULL DEFAULT 0,
    created_at INTEGER NOT NULL DEFAULT (unixepoch()),
    updated_at INTEGER NOT NULL DEFAULT (unixepoch())
  )`,
  `CREATE INDEX IF NOT EXISTS custom_charts_sort_idx ON custom_charts(favorite DESC, sort_order ASC, updated_at DESC)`,
]

/** 廃止されたテーブル。存在していれば DROP する（再実行しても無害）。 */
const DEPRECATED_DROPS: string[] = [
  `DROP TABLE IF EXISTS screener_snapshots`,
]

/** 既存テーブルに後から追加するカラム。重複エラーは握りつぶす。 */
const ADD_COLUMN_IF_MISSING: string[] = [
  `ALTER TABLE large_holder_position_certifications ADD COLUMN certification_method TEXT NOT NULL DEFAULT 'OTHER'`,
  `ALTER TABLE large_holder_position_certifications ADD COLUMN certification_strength TEXT NOT NULL DEFAULT 'NONE'`,
  `ALTER TABLE large_holder_position_certifications ADD COLUMN certification_evidence_json TEXT NOT NULL DEFAULT '[]'`,
  `ALTER TABLE large_holder_position_certifications ADD COLUMN pit_valuation_ready INTEGER NOT NULL DEFAULT 0`,
  `ALTER TABLE large_holder_price_evidence ADD COLUMN source_evidence TEXT`,
  `ALTER TABLE large_holder_filings ADD COLUMN report_serial_number INTEGER`,
  `ALTER TABLE large_holder_filings ADD COLUMN report_serial_source TEXT`,
  `ALTER TABLE large_holder_filings ADD COLUMN submission_count INTEGER`,
  `ALTER TABLE large_holder_positions ADD COLUMN security_breakdown_json TEXT NOT NULL DEFAULT '[]'`,
  `ALTER TABLE large_holder_positions ADD COLUMN deductions_json TEXT NOT NULL DEFAULT '[]'`,
  `ALTER TABLE large_holder_positions ADD COLUMN market_price_eligible_units REAL`,
  `ALTER TABLE large_holder_positions ADD COLUMN market_price_status TEXT NOT NULL DEFAULT 'NOT_APPLICABLE'`,
  `ALTER TABLE large_holder_positions ADD COLUMN market_price_reason TEXT NOT NULL DEFAULT 'not_assessed'`,
  `ALTER TABLE large_holder_positions ADD COLUMN market_price_basis_json TEXT NOT NULL DEFAULT '{}'`,
  `ALTER TABLE large_holder_positions ADD COLUMN market_price_evidence_json TEXT NOT NULL DEFAULT '{}'`,
  `ALTER TABLE large_holder_positions ADD COLUMN market_price_close_date TEXT`,
  `ALTER TABLE large_holder_positions ADD COLUMN market_price_close REAL`,
  `ALTER TABLE investor_entities ADD COLUMN classification_source TEXT`,
  `ALTER TABLE large_holder_filings ADD COLUMN filer_edinet_code TEXT`,
  `ALTER TABLE large_holder_filings ADD COLUMN filer_name TEXT`,
  `ALTER TABLE large_holder_filings ADD COLUMN previous_filing_id TEXT`,
  `ALTER TABLE large_holder_filings ADD COLUMN schema_regime TEXT NOT NULL DEFAULT 'UNKNOWN'`,
  `ALTER TABLE large_holder_filings ADD COLUMN root_filing_id TEXT`,
  `ALTER TABLE large_holder_filings ADD COLUMN revision_sequence INTEGER`,
  `ALTER TABLE large_holder_positions ADD COLUMN valuation_status TEXT NOT NULL DEFAULT 'NOT_PROVEN'`,
  `ALTER TABLE large_holder_positions ADD COLUMN valuation_evidence_json TEXT NOT NULL DEFAULT '[]'`,
  `ALTER TABLE large_holder_positions ADD COLUMN security_components_json TEXT NOT NULL DEFAULT '[]'`,
  `ALTER TABLE investor_entities ADD COLUMN classification_evidence TEXT`,
  `ALTER TABLE large_holder_filings ADD COLUMN parent_document_id TEXT`,
  `ALTER TABLE large_holder_filings ADD COLUMN withdrawn_at TEXT`,
  `ALTER TABLE trigger_evaluation_batch_items ADD COLUMN definition_name TEXT NOT NULL DEFAULT ''`,
  `ALTER TABLE trigger_evaluation_batch_items ADD COLUMN view_config_json TEXT NOT NULL DEFAULT '{"sort":null,"pageSize":50}'`,
  `ALTER TABLE notification_outbox ADD COLUMN delivery_profile_id TEXT`,
  `ALTER TABLE notification_outbox ADD COLUMN delivery_provider TEXT`,
  `ALTER TABLE notification_outbox ADD COLUMN provider_message_id TEXT`,
  `ALTER TABLE notification_outbox ADD COLUMN provider_thread_id TEXT`,
  `ALTER TABLE notification_outbox ADD COLUMN delivery_retryable INTEGER`,
  `ALTER TABLE notification_outbox ADD COLUMN delivery_started_at INTEGER`,
  `ALTER TABLE valuation_daily_serving ADD COLUMN peer_forward_per REAL`,
  `ALTER TABLE valuation_daily_serving ADD COLUMN peer_pbr REAL`,
  `ALTER TABLE valuation_daily_serving ADD COLUMN peer_fcf_yield REAL`,
  `ALTER TABLE valuation_daily_serving ADD COLUMN peer_roe REAL`,
  `ALTER TABLE valuation_daily_serving ADD COLUMN peer_revenue_growth REAL`,
  `ALTER TABLE valuation_daily_serving ADD COLUMN peer_ev_ebitda REAL`,
  `ALTER TABLE sector_master ADD COLUMN market_segment TEXT`,
  `ALTER TABLE sector_master ADD COLUMN sector33 TEXT`,
  `ALTER TABLE sector_master ADD COLUMN margin_type TEXT`,
  // Phase 3.6: 時価総額計算用
  `ALTER TABLE ticker_universe ADD COLUMN shares_outstanding INTEGER`,
  `ALTER TABLE ticker_universe ADD COLUMN shares_updated_at INTEGER`,
  // Phase 4: J-Quants /listed/info 由来の業種区分
  `ALTER TABLE ticker_universe ADD COLUMN sector17_code TEXT`,
  `ALTER TABLE ticker_universe ADD COLUMN sector17_name TEXT`,
  `ALTER TABLE ticker_universe ADD COLUMN sector33_code TEXT`,
  `ALTER TABLE ticker_universe ADD COLUMN sector33_name TEXT`,
  `ALTER TABLE ticker_universe ADD COLUMN market_segment TEXT`,
  `ALTER TABLE ticker_universe ADD COLUMN margin_code TEXT`,
  `ALTER TABLE ticker_universe ADD COLUMN margin_type TEXT`,
  // US: Tiingo調整OHLCをraw価格と分離して保持する
  `ALTER TABLE market_ohlcv_daily ADD COLUMN adj_open REAL`,
  `ALTER TABLE market_ohlcv_daily ADD COLUMN adj_high REAL`,
  `ALTER TABLE market_ohlcv_daily ADD COLUMN adj_low REAL`,
  `ALTER TABLE market_ohlcv_daily ADD COLUMN adj_volume INTEGER`,
  // US screener: latest-period metrics are computed once with the snapshot.
  `ALTER TABLE market_daily_snapshots ADD COLUMN prev_close REAL`,
  `ALTER TABLE market_daily_snapshots ADD COLUMN close_5d REAL`,
  `ALTER TABLE market_daily_snapshots ADD COLUMN close_20d REAL`,
  `ALTER TABLE market_daily_snapshots ADD COLUMN close_60d REAL`,
  `ALTER TABLE market_daily_snapshots ADD COLUMN close_120d REAL`,
  `ALTER TABLE market_daily_snapshots ADD COLUMN avg_volume_20 REAL`,
  `ALTER TABLE market_daily_snapshots ADD COLUMN ma_200 REAL`,
  `ALTER TABLE market_daily_snapshots ADD COLUMN ma_200_prev REAL`,
  `ALTER TABLE market_daily_snapshots ADD COLUMN ma_200_observations INTEGER`,
  // Phase 4: J-Quants + JPX 公式決算予定の表示補完
  `ALTER TABLE earnings_calendar ADD COLUMN company_name TEXT`,
  `ALTER TABLE earnings_calendar ADD COLUMN sector_name TEXT`,
  `ALTER TABLE earnings_calendar ADD COLUMN market_segment TEXT`,
  `ALTER TABLE earnings_calendar ADD COLUMN source TEXT`,
  `ALTER TABLE earnings_calendar ADD COLUMN source_url TEXT`,
  `ALTER TABLE earnings_calendar ADD COLUMN scheduled_time TEXT`,
  `ALTER TABLE earnings_calendar ADD COLUMN scheduled_time_kind TEXT`,
  `ALTER TABLE earnings_calendar ADD COLUMN scheduled_time_source TEXT`,
  `ALTER TABLE earnings_calendar ADD COLUMN scheduled_time_source_url TEXT`,
  `ALTER TABLE earnings_calendar ADD COLUMN predicted_time TEXT`,
  `ALTER TABLE earnings_calendar ADD COLUMN prediction_confidence TEXT`,
  `ALTER TABLE earnings_calendar ADD COLUMN prediction_sample_count INTEGER`,
  `ALTER TABLE earnings_calendar ADD COLUMN prediction_mode_count INTEGER`,
  `ALTER TABLE earnings_calendar ADD COLUMN actual_disclosed_date TEXT`,
  `ALTER TABLE earnings_calendar ADD COLUMN actual_disclosed_time TEXT`,
  `ALTER TABLE earnings_calendar ADD COLUMN actual_disclosed_at TEXT`,
  `ALTER TABLE earnings_calendar ADD COLUMN actual_source TEXT`,
  `ALTER TABLE earnings_calendar ADD COLUMN actual_source_url TEXT`,
  `ALTER TABLE earnings_calendar ADD COLUMN time_bucket TEXT`,
  `ALTER TABLE earnings_calendar ADD COLUMN time_updated_at INTEGER`,
  `CREATE INDEX IF NOT EXISTS earn_import_date_ticker_idx
    ON earnings_calendar(imported_at, announce_date, ticker)`,
  `CREATE INDEX IF NOT EXISTS earn_time_bucket_date_idx ON earnings_calendar(time_bucket, announce_date)`,
  `ALTER TABLE kabutan_material_news ADD COLUMN related_stocks_json TEXT NOT NULL DEFAULT '[]'`,
]

function isBusy(error: unknown): boolean {
  const message = error instanceof Error ? error.message : String(error)
  return /SQLITE_BUSY|database is locked/i.test(message)
}

function sleep(ms: number): Promise<void> {
  return new Promise((resolve) => setTimeout(resolve, ms))
}

async function executeSchemaSql(client: Client, sql: string): Promise<void> {
  const max = Number(process.env.SQLITE_BUSY_RETRIES ?? 8)
  for (let attempt = 0; ; attempt += 1) {
    try {
      await client.execute(sql)
      return
    } catch (error) {
      if (!isBusy(error) || attempt >= max) throw error
      await sleep(Math.min(2500, 120 * 2 ** attempt))
    }
  }
}

export async function ensureSchema(client: Client): Promise<void> {
  for (const sql of STATEMENTS) {
    await executeSchemaSql(client, sql)
  }
  for (const sql of DEPRECATED_DROPS) {
    try { await executeSchemaSql(client, sql) } catch { /* 無視 */ }
  }
  for (const sql of ADD_COLUMN_IF_MISSING) {
    try { await executeSchemaSql(client, sql) } catch (e) {
      const msg = (e as Error).message ?? ''
      if (!/duplicate column name/i.test(msg)) {
        // 致命的でないので警告のみ
        console.warn('[migrate] ADD COLUMN warning:', msg)
      }
    }
  }
}
