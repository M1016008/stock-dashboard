...
const dailyInstaller = read('scripts/install-local-daily-refresh.ts')
...
assert.equal(fs.existsSync(path.join(root, 'scripts/install-local-ma-trajectory.ts')), false)
assert.doesNotMatch(dailyInstaller, /com\.stockboard\.ma-trajectory/)
assert.doesNotMatch(dailyInstaller, /JP MA trajectory refresh/)
assert.doesNotMatch(packageJson, /auto-ma-trajectory:install/)
assert.match(jpDetail, /ScenarioProjectionChart/)
assert.match(usDetail, /ScenarioProjectionChart/)
assert.doesNotMatch(jpDetail, /FutureScenarioPanel/)
assert.doesNotMatch(usDetail, /FutureScenarioPanel/)
