...
assert.match(trainer, /pairedMaeDifferenceVsLightgbm95Ci/)
assert.match(trainer, /noMaterialHorizonRegression/)
assert.match(trainer, /noMaterialHorizonRegressionVsLightgbm/)
assert.match(trainer, /encoder_must_beat_euclidean_and_lightgbm/)
assert.match(trainer, /objective": "lambdarank"/)
assert.match(trainer, /lightgbm_pair_features/)
assert.match(trainer, /method, selected, scores/)
...
assert.match(runner, /ANALOG_ENCODER_LIGHTGBM_TRAIN_QUERIES/)
assert.match(runner, /encoder must beat both baselines/)
assert.match(runner, /productionRankingChanged: false/)

const requirements = read('requirements/analog-encoder.txt')
assert.match(requirements, /lightgbm>=4,<5/)
const setup = read('scripts/setup-analog-encoder-python.ts')
assert.match(setup, /canImportLightgbm/)
assert.match(setup, /USE_OPENMP=OFF/)
