...
async function main() {
  ...
  try {
    ...
    for (const [index, doc] of missing.entries()) {
      ...
      const bytes = await downloadEdinetPublicArchiveBytes(doc)
      ...
      const source = await archiveOfficialRaw(archiveRoot, 'EDINET',
      ...
      console.log(JSON.stringify({ fetched: index + 1, total: missing.length, documentId: doc }))
    ...
    console.log(JSON.stringify({ activityManifestPath: outputPath, baseManifestSha256: baseHash,
      supplementalOfficialDocuments: sources.length, dbWrites: 0 }))
  } finally { db.close() }
}
