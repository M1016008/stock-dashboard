...
assert.ok(client.includes('Zone下抜けも候補に含める'), 'Below Zone opt-in must be visible in Trigger details')
assert.ok(client.includes('label="最大下抜け幅"'), 'positive Below Zone magnitude must be editable only while enabled')
assert.ok(client.includes('value="BELOW_ZONE"'), 'Below Zone must be selectable as a result view filter')
assert.ok(client.includes('changeStatusFilter'), 'Status view filter must be applied to server-side pagination')
