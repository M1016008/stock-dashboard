...
const START_YEAR = Number(process.env.ML_PHYSICS_EVAL_START_YEAR ?? 1900)
...
const VALIDATION_LIMIT = Number(process.env.ML_PHYSICS_EVAL_VALIDATION_LIMIT ?? 25000)
const EPOCHS = Number(process.env.ML_PHYSICS_EVAL_EPOCHS ?? 40)
const LR = Number(process.env.ML_PHYSICS_EVAL_LR ?? 0.05)
...
function train(rows: Array<{ vector: number[]; label: number }>) {
  const weights = Array.from({ length: ML_PHYSICS_FEATURE_NAMES.length }, () => 0)
  let intercept = 0
...
async function loadRows(horizon: number, from: string, to: string, limit: number): Promise<LabeledRow[]> {
  return execAll<LabeledRow>(
    ...
    LIMIT ?
    `,
    [ML_PHYSICS_FEATURE_SET, horizon, from, to, limit],
  )
}
...
function labelFor(row: { up: number; down: number; wait: number }, direction: PhysicsDirection): number {
  if (direction === 'up') return row.up
  if (direction === 'down') return row.down
  return row.wait
}
...
async function evaluateFold(horizon: number, year: number): Promise<Array<{ sql: string; args: Array<string | number | null> }>> {
  ...
  const parsedTrain = trainRows
    .map((row) => ({ vector: parseVector(row.vector_json), up: row.up_label ? 1 : 0, down: row.down_label ? 1 : 0, wait: row.wait_label ? 1 : 0 }))
  ...
  for (const direction of ['up', 'down', 'wait'] as PhysicsDirection[]) {
    const model = train(parsedTrain.map((row) => ({ vector: row.vector, label: labelFor(row, direction) })))
    const scored = parsedValidation
      .map((row) => ({
        score: sigmoid(model.intercept + dot(model.weights, row.vector)),
        hit: labelFor(row, direction),
...
async function main() {
  ...
  for (const horizon of HORIZONS) {
    ...
    for (let year = START_YEAR; year <= endYear; year += 1) {
      ...
      if (statements.length >= 45) await execBatch(statements.splice(0, statements.length))
      console.log(`ml physics evaluate: horizon=${horizon}, year=${year}`)
    }
  }
  if (statements.length > 0) await execBatch(statements)
  console.log('ml physics evaluate complete')
}
