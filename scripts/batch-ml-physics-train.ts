...
const EPOCHS = Number(process.env.ML_PHYSICS_TRAIN_EPOCHS ?? 60)
const LR = Number(process.env.ML_PHYSICS_TRAIN_LR ?? 0.05)
...
async function trainHorizon(horizon: number): Promise<void> {
  ...
  for (const direction of ['up', 'down', 'wait'] as PhysicsDirection[]) {
    ...
    const modelName = `ma_physics_${direction}_h${horizon}_${MODEL_VERSION}`
    statements.push({
      sql: `
...
async function main() {
  console.log(`ml physics train: version=${MODEL_VERSION}, mode=${SAMPLE_MODE}, limit=${LIMIT || 'all'}, per_year=${PER_YEAR_LIMIT || 'auto'}, start=${START_DATE}, end=${END_DATE ?? 'auto'}, horizons=${HORIZONS.join('/')}`)
