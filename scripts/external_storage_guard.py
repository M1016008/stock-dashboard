...
class StorageGuard:
    def __init__(self, target, *, probe=volume_info, clock=time.monotonic):
    ...
    def fatal(self, error):
        if self.state != "STORAGE_FATAL":
            self.state = "STORAGE_FATAL"
            incident = {
            ...
            if self.connection:
                try:
                    ...
                    pass
        raise StorageFatal("STORAGE_UNAVAILABLE: writer has entered irreversible STORAGE_FATAL") from error
    ...
    def _check(self, identity):
        ...
        elif self.mount_real != real_mount or not os.access(real_mount, os.W_OK):
    ...
    def assert_writable(self, force=False):
        ...
        now = self.clock()
        identity = force or self.last_identity is None or now - self.last_identity >= 60
        try:
