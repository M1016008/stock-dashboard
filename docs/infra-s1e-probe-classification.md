...
- If SQLite removes a WAL/SHM companion between its existence and identity checks, Node's lightweight and full probes and the Python guard require full UUID recertification instead of treating the ordinary `ENOENT` race as a permanent storage failure. A still-present but unreadable companion remains fatal.
