...
import type { AssistantChatResponse, AssistantPageContext, AssistantResultRow } from '@/lib/assistant/types'
...
function assistantStatusText(status: AssistantStatusResponse | null): string {
...
export function AssistantDrawer() {
  ...
  const suggestions = useMemo(() => {
    ...
    return ['日経225で上昇候補を探して', '下落警戒が強い銘柄', '明日以降の決算候補を見せて']
  }, [pathname, ticker])
  ...
  const run = async (message: string) => {
    ...
    try {
      ...
      setResponse(json as AssistantChatResponse)
    } catch (e) {
        ...
        <div className="fixed bottom-5 right-5 z-50 flex max-h-[calc(100vh-40px)] w-[min(440px,calc(100vw-40px))] flex-col overflow-hidden rounded-[8px] border border-[var(--color-border-strong)] bg-white shadow-[0_18px_48px_rgba(16,32,52,0.30)]">
          <div className="flex items-center justify-between border-b border-[var(--color-border-default)] bg-[var(--color-brand-800)] px-4 py-3 text-white">
            <div className="flex items-center gap-2">
              ...
              <div>
                <div className="text-[13px] font-black">StockBoard AI</div>
                <div className="text-[10px] font-bold text-white/75">読み取り専用 / 既存機能を横断検索</div>
              </div>
            </div>
            <button
              type="button"
            ...
            {response && (
              <div className="mt-3 space-y-3">
                <div className="rounded-[6px] border border-[var(--color-border-default)] bg-white p-3">
                  <div className="mb-1 text-[12px] font-black text-[var(--color-text-primary)]">回答</div>
          ...
          <form onSubmit={onSubmit} className="border-t border-[var(--color-border-default)] bg-white p-3">
            <div className="flex items-end gap-2">
              <textarea
                ...
                placeholder="例: 日経225で下落警戒が強い貸借銘柄を出して"
            ...
            <div className="mt-2 text-[10px] font-bold text-[var(--color-text-tertiary)]">
              AIは登録済みの読み取り専用機能だけを呼び出します。投資判断は自己責任で確認してください。
            </div>
          </form>
        </div>
      )}
