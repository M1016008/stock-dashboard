...
export async function GET(
  ...
  try {
    const model = await getFinancialDetailReadModel(ticker, requestedAsOf)
    if (model.coverage.facts === 0) {
      return NextResponse.json({ error: 'financial_detail_not_found' }, { status: 404 })
    }
    ...
    console.error(`[financial-detail] ticker=${ticker}`, error)
    return NextResponse.json({ error: 'financial_detail_unavailable' }, { status: 500 })
  }
}
