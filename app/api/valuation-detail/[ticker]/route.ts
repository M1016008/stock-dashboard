...
export async function GET(
  ...
  try {
    const model = await getValuationDetailReadModel(ticker, requestedAsOf)
    if (!model.classification.name && model.coverage.priceObservations === 0) {
      return NextResponse.json({ error: 'valuation_detail_not_found' }, { status: 404 })
    }
    ...
    console.error(`[valuation-detail] ticker=${ticker}`, error)
    return NextResponse.json({ error: 'valuation_detail_unavailable' }, { status: 500 })
  }
}
