...
export async function GET(
  ...
  try {
    const model = await getFinancialPerformanceDetailReadModel(ticker, requestedAsOf)
    if (model.coverage.facts === 0) {
      return NextResponse.json({ error: 'performance_detail_not_found' }, { status: 404 })
    }
    ...
    console.error(`[financial-performance-detail] ticker=${ticker}`, error)
    return NextResponse.json({ error: 'financial_performance_detail_unavailable' }, { status: 500 })
  }
}
