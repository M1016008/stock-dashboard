...
export async function POST(request: Request) {
  try {
    const result = await compareOutcomeSegments(await request.json())
    ...
    console.error('[trigger-discovery/outcome-segments:compare]', error)
    return NextResponse.json({ error: 'outcome_segment_comparison_unavailable' }, { status: 500 })
  }
}
