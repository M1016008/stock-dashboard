...
export async function GET(
  ...
  try {
    const model = await getFinancialPerformanceTimelineReadModel(ticker, requestedAsOf)
    ...
    console.error(`[financial-performance-timeline] ticker=${ticker}`, error)
    return NextResponse.json({ error: 'financial_performance_timeline_unavailable' }, { status: 500 })
  }
}
